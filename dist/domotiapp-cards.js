...
`;function q(a){let e=new CSSStyleSheet;return e.replaceSync(a),e}var A=a=>String(a??"").split(".")[0],m=(a,e)=>e&&a?.states?.[e]||null,k=(a,e)=>m(a,e)?.attributes??{};function M(a,e,t){return t||k(a,e).friendly_name||e||""}var Fe=new Set(["scene","script","input_button","button","event"]),K=a=>Fe.has(A(a));function D(a){return!a||a.state==="unavailable"?!0:a.state==="unknown"?!K(a.entity_id):!1}function X(a){if(!a)return!1;let e=a.state;if(e==="unavailable"||e==="unknown")return!1;switch(A(a.entity_id)){case"cover":return e==="open"||e==="opening";case"alarm_control_panel":return e.startsWith("armed")||e==="triggered"||e==="arming";case"climate":case"water_heater":case"humidifier":return e!=="off";case"person":case"device_tracker":return e==="home";case"media_player":return e!=="off"&&e!=="idle"&&e!=="standby";default:return e==="on"||e==="playing"||e==="active"||e==="heat"}}function Ie(a,e,t){if(!a||a.themes!==e.themes||a.language!==e.language)return!0;for(let n of t)if(n&&a.states?.[n]!==e.states?.[n])return!0;return!1}function Z(a,e,t={}){a.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1}))}var N=(a,e)=>Z(a,"hass-more-info",{entityId:e});function Y(a){switch(A(a)){case"light":case"switch":case"fan":case"input_boolean":case"automation":case"siren":return{action:"toggle"};case"script":case"scene":case"input_button":case"button":return{action:"toggle"};default:return{action:"more-info"}}}function Ze(a){switch(A(a)){case"scene":return["scene","turn_on"];case"script":return["script","turn_on"];case"input_button":return["input_button","press"];case"button":return["button","press"];case"lock":return["lock","open"];case"cover":return["cover","toggle"];case"media_player":return["media_player","media_play_pause"];default:return["homeassistant","toggle"]}}function J(a,e,t,n){if(!(!n||n.action==="none"))switch(n.action){case"more-info":N(a,n.entity||t.entity);break;case"toggle":{let i=n.entity||t.entity;if(!i)break;let[r,s]=Ze(i);e.callService(r,s,{entity_id:i});break}case"perform-action":case"call-service":{let i=n.perform_action||n.service;if(!i)break;let[r,s]=i.split(".");e.callService(r,s,n.data??n.service_data??{},n.target);break}case"navigate":if(!n.navigation_path)break;history.pushState(null,"",n.navigation_path),Z(window,"location-changed",{replace:!1});break;case"url":n.url_path&&window.open(n.url_path,n.target??"_blank");break;case"assist":Z(a,"show-dialog",{dialogTag:"ha-voice-command-dialog",dialogImport:()=>{},dialogParams:{}});break;case"fire-dom-event":Z(a,"ll-custom",n);break;default:break}}function z(a,{onTap:e,onHold:t,onDouble:n}){let s=0,c=0,l=null,d=g=>{g.button!=null&&g.button!==0||(s=Date.now())},h=()=>{let g=s?Date.now()-s:0;if(s=0,t&&g>=500){navigator.vibrate?.(18),t();return}if(!n){e?.();return}if(c++,c===1){l=setTimeout(()=>{c=0,e?.()},260);return}clearTimeout(l),c=0,n()};return a.addEventListener("pointerdown",d),a.addEventListener("click",h),a.addEventListener("contextmenu",g=>g.preventDefault()),()=>{clearTimeout(l),a.removeEventListener("pointerdown",d),a.removeEventListener("click",h)}}function T(a,e){if(!e)return"";let t=A(e.entity_id),n=e.attributes.device_class;return a.formatEntityState?.(e)??a.localize?.(`component.${t}.entity_component.${n??"_"}.state.${e.state}`)??a.localize?.(`component.${t}.entity_component._.state.${e.state}`)??e.state}function L(a,e,t){let n=Number(e);return Number.isFinite(n)?n.toLocaleString(a?.locale?.language??"nl",{minimumFractionDigits:t??0,maximumFractionDigits:t??0}):"--"}var De=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],He=["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"],Ne=(a=new Date)=>new Date(a.getFullYear(),a.getMonth(),a.getDate()),ae=(a,e)=>Math.round((Ne(e)-Ne(a))/864e5);function G(a){if(!a)return null;if(a instanceof Date)return Number.isNaN(+a)?null:a;let e=String(a).trim(),t=e.match(/^(\d{1,2})-(\d{1,2})-(\d{4})/);if(t)return new Date(+t[3],+t[2]-1,+t[1]);if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/),t)return new Date(+t[1],+t[2]-1,+t[3]);let n=new Date(e);return Number.isNaN(+n)?null:n}function re(a,e=new Date){if(!a)return"";let t=ae(e,a);return t<0?`${Math.abs(t)} dagen geleden`:t===0?"vandaag":t===1?"morgen":t===2?"overmorgen":t<=6?De[a.getDay()]:`${De[a.getDay()].slice(0,2)} ${a.getDate()} ${He[a.getMonth()]}`}var Pe=a=>a?`${a.getDate()} ${He[a.getMonth()]}`:"";var Xe=`
  :host {
    ${P}
    display: block;
...
`,O={accent:"var(--dac-accent-hi)",solar:"var(--dac-solar)",house:"var(--dac-house)",water:"var(--dac-grid-in)",magenta:"var(--dac-grid-out)",pink:"var(--dac-device-1)",teal:"var(--dac-device-2)",lit:"var(--dac-lit)",good:"var(--dac-good)",warn:"var(--dac-warn)",bad:"var(--dac-bad)",neutral:"var(--dac-ink-3)"},V={accent:"Accent",solar:"Oranje",house:"Blauw",water:"Lichtblauw",magenta:"Magenta",pink:"Roze",teal:"Groenblauw",lit:"Lampgeel",good:"Goed",warn:"Let op",bad:"Kritiek",neutral:"Neutraal"},$=(a,e="accent")=>O[a]??(a&&/[#(]|^var/.test(a)?a:O[e]),E=Symbol("incomplete"),Ye=a=>`
  <div class="needs">
  ...
  </div>`,Je=56,qe=8,H=a=>Math.max(1,Math.ceil((a+qe)/(Je+qe))),v=class extends HTMLElement{static css="";static get styleSheets_(){return Object.hasOwn(this,"sheets_")||(this.sheets_=[q(Xe+je+this.css)]),this.sheets_}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=new.target.styleSheets_,this.built_=!1,this.wired_=!1,this.teardown_=[]}setConfig(e){let t=this.validate(e??{});this.config=t,this.built_&&(this.destroy_(),this.shadowRoot.replaceChildren(),this.built_=!1,this.wired_=!1),this.isConnected&&this.build_()}set hass(e){let t=this.hass_;if(this.hass_=e,!!this.config){if(!this.built_){this.build_();return}this.config[E]||Ie(t,e,this.watched())&&this.paint()}}get hass(){return this.hass_}connectedCallback(){if(this.config){if(!this.built_){this.build_();return}this.config[E]||this.wired_||(this.wire(),this.wired_=!0,this.hass_&&this.paint())}}disconnectedCallback(){this.destroy_(),this.wired_=!1}validate(e){return e}watched(){return this.config?.entity?[this.config.entity]:[]}template(){return""}wire(){}paint(){}build_(){let e=document.createElement("template"),t=this.config?.[E];e.innerHTML=t?Ye(t):this.template(),this.shadowRoot.appendChild(e.content),this.built_=!0,!t&&(this.wire(),this.wired_=!0,this.hass_&&this.paint())}destroy_(){for(let e of this.teardown_)try{e()}catch{}this.teardown_=[]}on(e,t,n,i){e&&(e.addEventListener(t,n,i),this.teardown_.push(()=>e.removeEventListener(t,n,i)))}$(e){return this.shadowRoot.querySelector(e)}$$(e){return[...this.shadowRoot.querySelectorAll(e)]}text(e,t){let n=typeof e=="string"?this.$(e):e;n&&n.textContent!==String(t)&&(n.textContent=t)}getCardSize(){return 1}};function y(a,e,{name:t,description:n,preview:i=!0}={}){customElements.get(a)||(customElements.define(a,e),window.customCards=window.customCards??[],window.customCards.push({type:a,name:t??a,description:n??"",preview:i,documentationURL:"https://github.com/Sven2410/domotiapp-cards"}))}function _(a,e){customElements.get(a)||customElements.define(a,e)}var o=(a,e="none")=>`<svg class="icon" viewBox="0 0 24 24" fill="${e}" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${a}</svg>`,f={house:o(`<path d="M3.2 11.3 12 4.1l8.8 7.2"/>
...
`;var lt=new Set(["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"]),dt=new Set(["hs","rgb","rgbw","rgbww","xy"]),_e=a=>a?.attributes?.supported_color_modes??[],ht=a=>_e(a).some(e=>lt.has(e)),ne=a=>_e(a).some(e=>dt.has(e)),ie=a=>_e(a).includes("color_temp"),Re=a=>Math.max(1,Math.round((a??0)/255*100)),we=class extends v{static css=`
    :host { display: block; }
  ...
  `;validate(e){let t=e.items??e.entities??[];return t.length?{columns:2,show_state:!0,...e,items:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Voeg minstens \xE9\xE9n entiteit toe."}}watched(){return this.config.items.map(e=>e.entity)}tone_(e){if(e.tone)return $(e.tone);if(this.config.tone)return $(this.config.tone);if(A(e.entity)!=="light")return O.accent;let t=m(this.hass,e.entity),n=t?.state==="on"?t.attributes?.rgb_color:null;return n?`rgb(${n[0]},${n[1]},${n[2]})`:O.lit}template(){let e=this.config;e.bare&&this.setAttribute("bare",""),this.style.containerType="inline-size";let t=e.items.map((i,r)=>`
      <div class="it" role="button" tabindex="0" data-i="${r}">
        <span class="chip"></span>
        <span class="txt"><span class="nm"></span><span class="st"></span></span>
      </div>`).join("");return`<div class="card surface"><div class="grid" style="--cols:${Math.min(Math.max(1,Number(e.columns)||2),3)}">${t}</div></div>`}wire(){this.$$(".it").forEach(e=>{let t=this.config.items[+e.dataset.i],n=(i,r)=>J(this,this.hass,t,t[i]??r);this.teardown_.push(z(e,{onTap:()=>n("tap_action",Y(t.entity)),onHold:()=>n("hold_action",{action:"more-info"})}))})}paint(){this.$$(".it").forEach(e=>{let t=this.config.items[+e.dataset.i],n=m(this.hass,t.entity),i=X(n),r=D(n);e.dataset.on=String(i),e.classList.toggle("unavailable",r);let s=this.tone_(t);e.style.setProperty("--tone",s);let c=e.querySelector(".chip"),l=t.icon||Q(t.entity,k(this.hass,t.entity));c.dataset.icon!==l&&(c.dataset.icon=l,c.innerHTML=w(l)),c.style.setProperty("--tone",i?s:"var(--dac-ink-3)");let d=M(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),d);let h=e.querySelector(".st");if(this.config.show_state===!1)h.textContent="";else if(r)h.textContent="Niet bereikbaar";else if(!n||K(n.entity_id))h.textContent="";else if(A(n.entity_id)==="light"&&i&&n.attributes.brightness!=null)h.textContent=`${Math.round(n.attributes.brightness/255*100)}%`;else{let g=n.attributes.unit_of_measurement;h.textContent=g?`${n.state} ${g}`:T(this.hass,n)}e.setAttribute("aria-label",`${d}${n?`, ${T(this.hass,n)}`:""}`)})}lines_(){let e=Math.min(Math.max(1,Number(this.config?.columns)||2),3);return Math.ceil((this.config?.items?.length??1)/e)}getCardSize(){return this.lines_()}getGridOptions(){let e=this.lines_(),t=H(12+e*Ge+(e-1)*Ue);return{columns:12,rows:t,min_columns:4,min_rows:t,max_rows:t}}static getConfigElement(){return document.createElement("domotiapp-entities-card-editor")}static getStubConfig(e,t){return{entities:(t??[]).slice(0,2),columns:2}}},Ee=class extends x{defaults(){return{columns:2,show_state:!0}}setConfig(e){let t={...e},n=(e.items??e.entities??[]).map(i=>typeof i=="string"?{entity:i}:i);t.entities=n.map(i=>i.entity),delete t.items;for(let i of n)i.name&&(t[`naam:${i.entity}`]=i.name),i.icon&&(t[`icoon:${i.entity}`]=i.icon),i.tone&&(t[`kleur:${i.entity}`]=i.tone);super.setConfig(t)}serialize(e){let t={...e},n=t.entities??[];t.items=n.map(i=>{let r={entity:i};return t[`naam:${i}`]&&(r.name=t[`naam:${i}`]),t[`icoon:${i}`]&&(r.icon=t[`icoon:${i}`]),t[`kleur:${i}`]&&(r.tone=t[`kleur:${i}`]),r}),delete t.entities;for(let i of Object.keys(t))/^(naam|icoon|kleur):/.test(i)&&delete t[i];return t}pickers(){return(this.config_?.entities??[]).filter(t=>typeof t=="string").flatMap(t=>[{key:`icoon:${t}`,kind:"icon",label:`Icoon voor ${this.short_(t)}`,fallback:"star"},{key:`kleur:${t}`,kind:"tone",label:`Kleur voor ${this.short_(t)}`}])}short_(e){return this.hass?.states?.[e]?.attributes?.friendly_name??e}schema(){let e=(this.config_?.entities??[]).filter(t=>typeof t=="string");return[{name:"entities",selector:{entity:{multiple:!0}}},{name:"columns",selector:p.select([{value:1,label:"1 per rij"},{value:2,label:"2 naast elkaar"},{value:3,label:"3 naast elkaar"}])},{name:"show_state",selector:p.bool()},...e.map(t=>({name:`naam:${t}`,selector:p.text()}))]}label(e){return e.name.startsWith("naam:")?`Naam voor ${this.short_(e.name.slice(5))}`:{entities:"Entiteiten",columns:"Kolommen",show_state:"Toestand tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="entities")return"Per entiteit kun je hieronder een eigen icoon, kleur en naam zetten.";if(e.name==="columns")return"Onder de 260 pixels valt de kaart vanzelf terug op \xE9\xE9n kolom."}};_("domotiapp-entities-card-editor",Ee);y("domotiapp-entities-card",Se,{name:"DomotiApp Entiteiten",description:"Een lijst entiteiten, per \xE9\xE9n, twee of drie naast elkaar."});var U={OPEN:1,CLOSE:2,SET_POSITION:4,STOP:8},B=(a,e)=>!!((a?.attributes?.supported_features??0)&e),ut=(a={})=>{switch(a.device_class){case"garage":return{open:"garageOpen",closed:"garageClosed"};case"awning":case"blind":return{open:"awning",closed:"awning"};default:return{open:"shutterOpen",closed:"shutter"}}},ze=class extends v{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.covers??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,covers:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Kies minstens \xE9\xE9n rolluik of zonnescherm."}}watched(){return this.config.covers.map(e=>e.entity)}keysHtml(e){return`
      ...
      </div>`).join("")}</div>`}wire(){this.dragging_=new Set,this.bound_=new Set,this.assumed_=new Map,this.$$(".cv").forEach(e=>{let t=e.dataset.i;e.querySelectorAll(".keys button").forEach(i=>{this.on(i,"click",()=>{let r=i.dataset.act,s={open:"open_cover",stop:"stop_cover",close:"close_cover"};this.hass.callService("cover",s[r],{entity_id:this.config.covers[+t].entity}),r!=="stop"&&(this.assumed_.set(t,r==="open"?"open":"closed"),this.paint())})});let n=this.config.covers[+t].entity;this.teardown_.push(z(e.querySelector(".chip"),{onTap:()=>N(this,n)}))})}paint(){this.$$(".cv").forEach(e=>{let t=e.dataset.i,n=this.config.covers[+t],i=m(this.hass,n.entity),r=k(this.hass,n.entity),s=!i||i.state==="unavailable",c=i?.state??"unknown";e.classList.toggle("unavailable",s),e.querySelector(".nm").textContent=M(this.hass,n.entity,n.name);let l=B(i,U.SET_POSITION)&&r.current_position!=null,d=l?r.current_position>0?"open":"closed":c==="open"||c==="closed"?c:this.assumed_.get(t)??"closed";e.dataset.shown=d;let h=ut(r),g=(d==="open"?n.icon_open:n.icon_closed)??(d==="open"?this.config.icon_open:this.config.icon_closed)??n.icon??h[d],b=e.querySelector(".chip");b.dataset.icon!==g&&(b.dataset.icon=g,b.innerHTML=w(g,h[d]));let I=e.querySelector(".st");this.dragging_.has(t)||(I.textContent=s?"Niet bereikbaar":c==="opening"?"Gaat open":c==="closing"?"Gaat dicht":l?`${r.current_position}% open`:c==="open"?"Open":c==="closed"?"Dicht":""),e.querySelectorAll(".keys button").forEach(S=>{if(S.dataset.act==="stop"){S.disabled=s||!B(i,U.STOP);return}let C=S.dataset.act==="open";S.disabled=s||(C?!B(i,U.OPEN):!B(i,U.CLOSE))});let u=e.querySelector(".pos"),j=l&&this.config.show_position!==!1;if(u.hidden=!j,j){if(u.dataset.built||(u.dataset.built="1",u.innerHTML=R("position"),u.querySelector(".slider").setAttribute("aria-label","Positie")),!this.bound_.has(t)){this.bound_.add(t);let C=u.querySelector(".slider"),F=W=>{C.style.setProperty("--v",`${W}%`),C.setAttribute("aria-valuenow",String(W)),e.querySelector(".st").textContent=`${W}% open`};this.teardown_.push(ee(C,{value:()=>k(this.hass,n.entity).current_position??0,onInput:F,onCommit:W=>this.hass.callService("cover","set_cover_position",{entity_id:n.entity,position:W})}))}let S=u.querySelector(".slider");if(!S.classList.contains("dragging")){let C=r.current_position??0;S.style.setProperty("--v",`${C}%`),S.setAttribute("aria-valuenow",String(C))}}})}rows_(){let e=this.config?.covers??[],t=e.some(n=>B(m(this.hass,n.entity),U.SET_POSITION));return H(12+Math.max(1,e.length)*42+(t?30:0))}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-cover-card-editor")}static getStubConfig(e,t){let n=t?.find(i=>i.startsWith("cover."));return{covers:n?[n]:[]}}},Ce=class extends x{defaults(){return{show_stop:!0,show_position:!0}}pickers(){return[{key:"icon_open",kind:"icon",label:"Icoon als het open staat",fallback:"shutterOpen"},{key:"icon_closed",kind:"icon",label:"Icoon als het dicht is",fallback:"shutter"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"covers",selector:{entity:{domain:"cover",multiple:!0}}},{name:"show_stop",selector:p.bool()}]}label(e){return{covers:"Rolluiken",show_stop:"Stopknop tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="covers")return"Melden ze hun stand terug, dan komt er vanzelf een schuif bij. Zo niet, dan blijven het open, stop en dicht, en volgt het icoon de knop die je indrukt."}};_("domotiapp-cover-card-editor",Ce);y("domotiapp-cover-card",ze,{name:"DomotiApp Rolluiken",description:"Open, stop en dicht, met een eigen icoon voor open en dicht."});function mt(a){if(!a)return{label:"Onbekend",home:null};switch(a.state){case"home":return{label:"Thuis",home:!0};case"not_home":return{label:"Afwezig",home:!1};case"unknown":case"unavailable":return{label:"Onbekend",home:null};default:return{label:a.state,home:!1}}}var Oe=class extends v{static css=`
    :host { display: block; height: 100%; }
