/*! DomotiApp Cards 0.1.6 | MIT | https://github.com/Sven2410/domotiapp-cards */
var P=`
  --dac-bg:            #0c0c0a;
  --dac-bg-raise:      #12120f;
...
`;function q(i){let e=new CSSStyleSheet;return e.replaceSync(i),e}var A=i=>String(i??"").split(".")[0],m=(i,e)=>e&&i?.states?.[e]||null,k=(i,e)=>m(i,e)?.attributes??{};function M(i,e,t){return t||k(i,e).friendly_name||e||""}var Fe=new Set(["scene","script","input_button","button","event"]),K=i=>Fe.has(A(i));function D(i){return!i||i.state==="unavailable"?!0:i.state==="unknown"?!K(i.entity_id):!1}function X(i){if(!i)return!1;let e=i.state;if(e==="unavailable"||e==="unknown")return!1;switch(A(i.entity_id)){case"cover":return e==="open"||e==="opening";case"alarm_control_panel":return e.startsWith("armed")||e==="triggered"||e==="arming";case"climate":case"water_heater":case"humidifier":return e!=="off";case"person":case"device_tracker":return e==="home";case"media_player":return e!=="off"&&e!=="idle"&&e!=="standby";default:return e==="on"||e==="playing"||e==="active"||e==="heat"}}function Ie(i,e,t){if(!i||i.themes!==e.themes||i.language!==e.language)return!0;for(let n of t)if(n&&i.states?.[n]!==e.states?.[n])return!0;return!1}function Z(i,e,t={}){i.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1}))}var N=(i,e)=>Z(i,"hass-more-info",{entityId:e});function Y(i){switch(A(i)){case"light":case"switch":case"fan":case"input_boolean":case"automation":case"siren":return{action:"toggle"};case"script":case"scene":case"input_button":case"button":return{action:"toggle"};default:return{action:"more-info"}}}function Ze(i){switch(A(i)){case"scene":return["scene","turn_on"];case"script":return["script","turn_on"];case"input_button":return["input_button","press"];case"button":return["button","press"];case"lock":return["lock","open"];case"cover":return["cover","toggle"];case"media_player":return["media_player","media_play_pause"];default:return["homeassistant","toggle"]}}function J(i,e,t,n){if(!(!n||n.action==="none"))switch(n.action){case"more-info":N(i,n.entity||t.entity);break;case"toggle":{let a=n.entity||t.entity;if(!a)break;let[r,s]=Ze(a);e.callService(r,s,{entity_id:a});break}case"perform-action":case"call-service":{let a=n.perform_action||n.service;if(!a)break;let[r,s]=a.split(".");e.callService(r,s,n.data??n.service_data??{},n.target);break}case"navigate":if(!n.navigation_path)break;history.pushState(null,"",n.navigation_path),Z(window,"location-changed",{replace:!1});break;case"url":n.url_path&&window.open(n.url_path,n.target??"_blank");break;case"assist":Z(i,"show-dialog",{dialogTag:"ha-voice-command-dialog",dialogImport:()=>{},dialogParams:{}});break;case"fire-dom-event":Z(i,"ll-custom",n);break;default:break}}function C(i,{onTap:e,onHold:t,onDouble:n}){let s=0,c=0,l=null,d=g=>{g.button!=null&&g.button!==0||(s=Date.now())},h=()=>{let g=s?Date.now()-s:0;if(s=0,t&&g>=500){navigator.vibrate?.(18),t();return}if(!n){e?.();return}if(c++,c===1){l=setTimeout(()=>{c=0,e?.()},260);return}clearTimeout(l),c=0,n()};return i.addEventListener("pointerdown",d),i.addEventListener("click",h),i.addEventListener("contextmenu",g=>g.preventDefault()),()=>{clearTimeout(l),i.removeEventListener("pointerdown",d),i.removeEventListener("click",h)}}function T(i,e){if(!e)return"";let t=A(e.entity_id),n=e.attributes.device_class;return i.formatEntityState?.(e)??i.localize?.(`component.${t}.entity_component.${n??"_"}.state.${e.state}`)??i.localize?.(`component.${t}.entity_component._.state.${e.state}`)??e.state}function L(i,e,t){let n=Number(e);return Number.isFinite(n)?n.toLocaleString(i?.locale?.language??"nl",{minimumFractionDigits:t??0,maximumFractionDigits:t??0}):"--"}var De=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],He=["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"],Ne=(i=new Date)=>new Date(i.getFullYear(),i.getMonth(),i.getDate()),ae=(i,e)=>Math.round((Ne(e)-Ne(i))/864e5);function G(i){if(!i)return null;if(i instanceof Date)return Number.isNaN(+i)?null:i;let e=String(i).trim(),t=e.match(/^(\d{1,2})-(\d{1,2})-(\d{4})/);if(t)return new Date(+t[3],+t[2]-1,+t[1]);if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/),t)return new Date(+t[1],+t[2]-1,+t[3]);let n=new Date(e);return Number.isNaN(+n)?null:n}function re(i,e=new Date){if(!i)return"";let t=ae(e,i);return t<0?`${Math.abs(t)} dagen geleden`:t===0?"vandaag":t===1?"morgen":t===2?"overmorgen":t<=6?De[i.getDay()]:`${De[i.getDay()].slice(0,2)} ${i.getDate()} ${He[i.getMonth()]}`}var Pe=i=>i?`${i.getDate()} ${He[i.getMonth()]}`:"";var Xe=`
  :host {
    ${P}
    display: block;
...
`,O={accent:"var(--dac-accent-hi)",solar:"var(--dac-solar)",house:"var(--dac-house)",water:"var(--dac-grid-in)",magenta:"var(--dac-grid-out)",pink:"var(--dac-device-1)",teal:"var(--dac-device-2)",lit:"var(--dac-lit)",good:"var(--dac-good)",warn:"var(--dac-warn)",bad:"var(--dac-bad)",neutral:"var(--dac-ink-3)"},V={accent:"Accent",solar:"Oranje",house:"Blauw",water:"Lichtblauw",magenta:"Magenta",pink:"Roze",teal:"Groenblauw",lit:"Lampgeel",good:"Goed",warn:"Let op",bad:"Kritiek",neutral:"Neutraal"},$=(i,e="accent")=>O[i]??(i&&/[#(]|^var/.test(i)?i:O[e]),E=Symbol("incomplete"),Ye=i=>`
  <div class="needs">
  ...
  </div>`,Je=56,qe=8,H=i=>Math.max(1,Math.ceil((i+qe)/(Je+qe))),v=class extends HTMLElement{static css="";static get styleSheets_(){return Object.hasOwn(this,"sheets_")||(this.sheets_=[q(Xe+je+this.css)]),this.sheets_}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=new.target.styleSheets_,this.built_=!1,this.wired_=!1,this.teardown_=[]}setConfig(e){let t=this.validate(e??{});this.config=t,this.built_&&(this.destroy_(),this.shadowRoot.replaceChildren(),this.built_=!1,this.wired_=!1),this.isConnected&&this.build_()}set hass(e){let t=this.hass_;if(this.hass_=e,!!this.config){if(!this.built_){this.build_();return}this.config[E]||Ie(t,e,this.watched())&&this.paint()}}get hass(){return this.hass_}connectedCallback(){if(this.config){if(!this.built_){this.build_();return}this.config[E]||this.wired_||(this.wire(),this.wired_=!0,this.hass_&&this.paint())}}disconnectedCallback(){this.destroy_(),this.wired_=!1}validate(e){return e}watched(){return this.config?.entity?[this.config.entity]:[]}template(){return""}wire(){}paint(){}build_(){let e=document.createElement("template"),t=this.config?.[E];e.innerHTML=t?Ye(t):this.template(),this.shadowRoot.appendChild(e.content),this.built_=!0,!t&&(this.wire(),this.wired_=!0,this.hass_&&this.paint())}destroy_(){for(let e of this.teardown_)try{e()}catch{}this.teardown_=[]}$(e){return this.shadowRoot.querySelector(e)}$$(e){return[...this.shadowRoot.querySelectorAll(e)]}text(e,t){let n=typeof e=="string"?this.$(e):e;n&&n.textContent!==String(t)&&(n.textContent=t)}getCardSize(){return 1}};function y(i,e,{name:t,description:n,preview:a=!0}={}){customElements.get(i)||(customElements.define(i,e),window.customCards=window.customCards??[],window.customCards.push({type:i,name:t??i,description:n??"",preview:a,documentationURL:"https://github.com/Sven2410/domotiapp-cards"}))}function _(i,e){customElements.get(i)||customElements.define(i,e)}var o=(i,e="none")=>`<svg class="icon" viewBox="0 0 24 24" fill="${e}" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${i}</svg>`,f={house:o(`<path d="M3.2 11.3 12 4.1l8.8 7.2"/>
    ...
    <path d="M9.6 9.6a2.4 2.4 0 1 1 3.2 2.3c-.5.2-.8.7-.8 1.2v.6M12 16.6v.1"/>`)};function w(i,e="question"){return i?f[i]?f[i]:i.includes(":")?`<ha-icon class="icon" icon="${i}"></ha-icon>`:f[e]??f.question:f[e]??f.question}function Q(i,e={}){switch(String(i??"").split(".")[0]){case"light":return"bulb";case"switch":return"switchOn";case"cover":return e.device_class==="awning"||e.device_class==="blind"?"awning":"shutter";case"person":case"device_tracker":return"person";case"climate":return"thermo";case"binary_sensor":return e.device_class==="smoke"?"smoke":"shield";case"alarm_control_panel":return"shield";case"scene":case"script":case"input_button":case"button":return"star";case"weather":return"cloudSun";default:return"question"}}function Ve(i){switch(i){case"sunny":case"clear-night":return"sun";case"partlycloudy":return"cloudSun";case"cloudy":return"cloud";case"rainy":case"pouring":case"hail":case"lightning":case"lightning-rainy":return"rain";case"snowy":case"snowy-rainy":return"snow";case"fog":return"fog";case"windy":case"windy-variant":return"wind";default:return"cloud"}}var Qe=[["Woning",["house","floorB","floor1","floor2","garage","door","window","grid"]],["Rolluiken",["shutter","shutterOpen","awning","garageOpen","garageClosed","arrowUp","arrowDown","stop"]],["Licht en stroom",["bulb","bulbGroup","switchOn","power","plug","bolt","battery"]],["Personen",["person","people","away"]],["Apparaten",["tv","speaker","camera","car","washer","dishwasher","printer","fan","airco","radio"]],["Afval",["bin","binWheeled","calendar"]],["Weer",["sun","cloud","cloudSun","rain","snow","fog","wind","drop","uv","sunrise","sunset","thermo"]],["Status",["shield","lock","lockOpen","key","wifi","smoke","warning","check","close","clock","gaugeArrow"]],["Overig",["star","moon","leaf","cog","dots","plus","minus","chevronRight","chevronDown","question"]]],et=`
...
`,oe=null,se=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),oe=oe??[q(et)],this.shadowRoot.adoptedStyleSheets=oe,this.value_="",this.label="Icoon",this.fallback="question",this.auto=!0}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}connectedCallback(){this.built_||(this.built_=!0,this.build_())}build_(){this.shadowRoot.innerHTML=`
      <div class="label"></div>
      <div class="box">
        ...
        <div class="panel">
          ${Qe.map(([t,n])=>`
            <div class="group">
              <h4>${t}</h4>
              <div class="grid">
                ${n.map(a=>`<button type="button" class="opt" data-icon="${a}" title="${a}" aria-pressed="false">${f[a]??""}</button>`).join("")}
              </div>
            </div>`).join("")}
          <div class="mdi">
...
`,ce=null,le=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),ce=ce??[q(it)],this.shadowRoot.adoptedStyleSheets=ce,this.value_="",this.label="Kleur",this.statuses=!1}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}connectedCallback(){this.built_||(this.built_=!0,this.build_())}swatch(e){return`<button type="button" class="sw" data-tone="${e}" style="--c:${O[e]}"
      ...
      <div class="box">
        ...
        <div class="row">
          ...
          ${tt.map(e=>this.swatch(e)).join("")}
        </div>
        ${this.statuses?`<h4>Status</h4>
               <div class="row">${nt.map(e=>this.swatch(e)).join("")}</div>
               <p class="note">
  ...
  `;validate(e){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768,...e}}watched(){let e=this.config;return[e.weather,e.weather_uv,e.sun,e.precipitation_entity].filter(Boolean)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),e.show_rule===!1&&this.setAttribute("no-rule",""),`
      ...
      </div>`}wire(){let e=()=>{let n=6e4-Date.now()%6e4+50;this.timer_=setTimeout(()=>{this.paintClock_(),e()},n)};e(),this.teardown_.push(()=>clearTimeout(this.timer_));let t=Number(this.config.hide_below)||0;if(t>0){let n=matchMedia(`(max-width: ${t-1}px)`),a=()=>this.toggleAttribute("narrow",n.matches);a(),n.addEventListener("change",a),this.teardown_.push(()=>n.removeEventListener("change",a))}}paintClock_(){let e=new Date,t=this.config.name??this.hass?.user?.name??"",n=at(e);this.$(".hello").innerHTML=t?`${n}, <b>${t}</b>`:n,this.text(".date",`${rt[e.getDay()]} ${e.getDate()} ${ot[e.getMonth()]}`);let a=this.$(".clock");a&&this.text(a,e.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"}))}paint(){this.paintClock_();let e=this.config,t=m(this.hass,e.weather),n=k(this.hass,e.weather),a=this.$(".now");if(a&&t){let l=Ve(t.state);a.style.setProperty("--wtone",$(e.tone,"water"));let d=this.hass?.config?.unit_system?.temperature??"\xB0C";this.$(".temp").innerHTML=n.temperature!=null?`${L(this.hass,n.temperature,0)}<span>${d}</span>`:"--";let h=a.querySelector(".ic");h.dataset.icon!==l&&(h.dataset.icon=l,h.innerHTML=w(l,"cloud")),this.text(a.querySelector(".cond"),T(this.hass,t))}let r=this.$(".chips");if(!r)return;let s=st.map(l=>this.chip_(l,n)).filter(Boolean),c=s.map(l=>`${l.key}${l.value}`).join("|");r.dataset.sig!==c&&(r.dataset.sig=c,r.innerHTML=s.map(l=>`<span class="chip2" style="--tone:${$(pe[l.key].tone)}" title="${pe[l.key].label}">
             ${f[pe[l.key].icon]??""}${l.value}
           </span>`).join(""))}chip_(e,t){let n=this.config;switch(e){case"humidity":return t.humidity!=null?{key:e,value:`${Math.round(t.humidity)}%`}:null;case"wind":{if(t.wind_speed==null)return null;let a=this.hass?.config?.unit_system?.wind_speed??"km/h",r=ct(t.wind_bearing);return{key:e,value:`${L(this.hass,t.wind_speed,0)} ${a}${r?` ${r}`:""}`}}case"uv":{let r=k(this.hass,n.weather_uv).uv_index??t.uv_index??(n.weather_uv?Number(m(this.hass,n.weather_uv)?.state):null);return r!=null&&!Number.isNaN(+r)?{key:e,value:`UV ${L(this.hass,r,1)}`}:null}case"precipitation":{let a=m(this.hass,n.precipitation_entity);if(a){let r=Number(a.state);if(Number.isNaN(r))return null;let s=a.attributes.unit_of_measurement??"mm";return{key:e,value:`${L(this.hass,r,1)} ${s}`}}return t.precipitation!=null&&!Number.isNaN(+t.precipitation)?{key:e,value:`${L(this.hass,t.precipitation,1)} mm`}:null}case"pressure":return t.pressure!=null?{key:e,value:`${L(this.hass,t.pressure,0)} ${t.pressure_unit??"hPa"}`}:null;case"sunset":case"sunrise":{let r=m(this.hass,n.sun)?.attributes?.[e==="sunset"?"next_setting":"next_rising"];if(!r)return null;let s=new Date(r);return Number.isNaN(+s)?null:{key:e,value:s.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"})}}default:return null}}getCardSize(){return 2}getGridOptions(){return{columns:"full",rows:2,min_rows:2,max_rows:2}}static getConfigElement(){return document.createElement("domotiapp-header-card-editor")}static getStubConfig(e){return{weather:Object.keys(e?.states??{}).find(n=>n.startsWith("weather.")),sun:"sun.sun"}}},me=class extends x{defaults(){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768}}pickers(){return[{key:"tone",kind:"tone",label:"Kleur weericoon"}]}schema(){return[de({name:"weather",selector:p.entity("weather")},{name:"weather_uv",selector:{entity:{domain:["weather","sensor"]}}}),de({name:"sun",selector:p.entity("sun")},{name:"precipitation_entity",selector:p.entity("sensor")}),{name:"name",selector:p.text()},{name:"hide_below",selector:p.number(0,1400,8)}]}label(e){return{weather:"Weer (temperatuur, wind)",weather_uv:"Tweede weerbron (UV-index)",precipitation_entity:"Neerslagsensor",show_rule:"Accentlijn tonen",hide_below:"Verbergen onder breedte (px)",bare:"Zonder kaartrand",name:"Naam"}[e.name]??super.label(e)}helper(e){if(e.name==="weather_uv")return"Alleen voor de UV-index. Handig als je hoofdbron die niet meelevert.";if(e.name==="precipitation_entity")return"Een sensor in mm of mm/h, bijvoorbeeld neerslagintensiteit of regen laatste uur.";if(e.name==="hide_below")return"768 verbergt de header op telefoons en houdt hem op tablets en desktops. 0 zet het uit.";if(e.name==="name")return"Leeg laten voor de naam van de ingelogde gebruiker."}};_("domotiapp-header-card-editor",me);y("domotiapp-header-card",ue,{name:"DomotiApp Header",description:"Smalle strip met begroeting, weer en klok. Verbergt zichzelf op telefoons."});var ge=class extends v{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return{layout:"row",show_state:!0,show_name:!0,show_icon:!0,...e}}watched(){return[this.config.entity].filter(Boolean)}tone_(){let e=this.config;if(e.tone)return $(e.tone);if(A(e.entity)!=="light")return O.accent;let t=m(this.hass,e.entity),n=t?.state==="on"?t.attributes?.rgb_color:null;return n?`rgb(${n[0]},${n[1]},${n[2]})`:O.lit}template(){let e=this.config;return this.setAttribute("layout",["row","tile","compact"].includes(e.layout)?e.layout:"row"),`
      ...
      </div>`}wire(){let e=this.config,t=(a,r)=>J(this,this.hass,e,e[a]??r);this.teardown_.push(C(this.$(".btn"),{onTap:()=>t("tap_action",{action:e.entity?"more-info":"none"}),onHold:()=>t("hold_action",{action:e.entity?"more-info":"none"}),onDouble:e.double_tap_action?()=>t("double_tap_action",{action:"none"}):void 0}));let n=this.$(".chip");n&&(this.teardown_.push(C(n,{onTap:a=>t("icon_tap_action",Y(e.entity)),onHold:()=>t("icon_hold_action",{action:e.entity?"more-info":"none"})})),n.addEventListener("click",a=>a.stopPropagation()),n.addEventListener("pointerdown",a=>a.stopPropagation()))}paint(){let e=this.config,t=m(this.hass,e.entity),n=X(t),a=!!e.entity&&D(t);this.toggleAttribute("on",n),this.$(".btn").classList.toggle("unavailable",a),this.$(".btn").style.setProperty("--tone",this.tone_());let r=this.$(".chip");if(r){let c=e.icon||Q(e.entity,k(this.hass,e.entity));r.dataset.icon!==c&&(r.dataset.icon=c,r.innerHTML=w(c)),r.style.setProperty("--tone",n?this.tone_():"var(--dac-ink-3)"),r.setAttribute("aria-label",e.entity?`${M(this.hass,e.entity,e.name)} schakelen`:"Icoon")}this.text(".nm",M(this.hass,e.entity,e.name));let s=this.$(".st");s&&this.text(s,this.secondary_(t,a)),this.$(".btn").setAttribute("aria-label",`${M(this.hass,e.entity,e.name)}${t?`, ${T(this.hass,t)}`:""}`)}secondary_(e,t){return t?"Niet bereikbaar":!e||K(e.entity_id)?"":A(e.entity_id)==="light"&&e.state==="on"&&e.attributes.brightness!=null?`${Math.round(e.attributes.brightness/255*100)}%`:T(this.hass,e)}getCardSize(){return this.config?.layout==="tile"?2:1}getGridOptions(){return this.config?.layout==="tile"?{columns:6,rows:2,min_columns:3,min_rows:2,max_rows:2}:{columns:12,rows:1,min_columns:4,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-button-card-editor")}static getStubConfig(e,t){return{entity:t?.find(a=>a.startsWith("light."))??t?.find(a=>a.startsWith("switch."))??t?.[0],layout:"row"}}},ve=class extends x{defaults(){return{layout:"row",show_state:!0,show_name:!0,show_icon:!0,icon_tap_action:{action:"toggle"},tap_action:{action:"more-info"}}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"star"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"entity",selector:p.entity()},{name:"name",selector:p.text()},{name:"layout",selector:p.select([{value:"row",label:"Rij"},{value:"tile",label:"Tegel"},{value:"compact",label:"Compact"}])},{name:"show_icon",selector:p.bool()},{name:"show_name",selector:p.bool()},{name:"show_state",selector:p.bool()},{name:"icon_tap_action",selector:p.action("toggle")},{name:"icon_hold_action",selector:p.action("more-info")},{name:"tap_action",selector:p.action("more-info")},{name:"hold_action",selector:p.action("more-info")},{name:"double_tap_action",selector:p.action("none")}]}label(e){return{entity:"Entiteit",name:"Naam (overschrijft die van de entiteit)",layout:"Vorm",show_icon:"Icoon tonen",show_name:"Naam tonen",show_state:"Toestand tonen",icon_tap_action:"Tikken op het icoon",icon_hold_action:"Vasthouden op het icoon",tap_action:"Tikken op de kaart",hold_action:"Vasthouden op de kaart",double_tap_action:"Dubbeltikken op de kaart"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"Mag leeg blijven: zonder entiteit wordt dit een navigatieknop.";if(e.name==="icon_tap_action")return"Handig: het icoon schakelt de lichtgroep, de kaart navigeert naar de ruimte.";if(e.name==="tap_action")return"Wat er gebeurt als je naast het icoon tikt, bijvoorbeeld navigeren naar een pop-up."}};_("domotiapp-button-card-editor",ve);y("domotiapp-button-card",be,{name:"DomotiApp Knop",description:"E\xE9n control als rij, tegel of compacte pil. Vervangt tile, mushroom-entity en bubble-knoppen."});var xe=(i,e,t)=>Math.min(t,Math.max(e,i));function ee(i,e){let t=e.min??0,n=e.max??100,a=e.step??1,r=!1,s=u=>{let j=i.getBoundingClientRect();if(!j.width)return t;let S=xe((u-j.left)/j.width,0,1),z=t+S*(n-t);return xe(Math.round(z/a)*a,t,n)},c=u=>{try{i.setPointerCapture?.(u)}catch{}},l=u=>{try{i.hasPointerCapture?.(u)&&i.releasePointerCapture(u)}catch{}},d=u=>{e.disabled?.()||u.button!=null&&u.button!==0||(r=!0,c(u.pointerId),i.classList.add("dragging"),e.onInput(s(u.clientX)),u.preventDefault())},h=u=>{r&&(e.onInput(s(u.clientX)),u.preventDefault())},g=u=>{r&&(r=!1,l(u.pointerId),i.classList.remove("dragging"),e.onCommit(s(u.clientX)))},b=u=>{r&&(r=!1,l(u?.pointerId),i.classList.remove("dragging"),e.onInput(e.value()))},I=u=>{if(e.disabled?.())return;let j=(n-t)/10,S={ArrowLeft:-a,ArrowDown:-a,ArrowRight:a,ArrowUp:a,PageDown:-j,PageUp:j,Home:-1/0,End:1/0};if(!(u.key in S))return;u.preventDefault();let z=e.value(),F=xe(S[u.key]===-1/0?t:S[u.key]===1/0?n:z+S[u.key],t,n);e.onInput(F),e.onCommit(F)};return i.addEventListener("pointerdown",d),i.addEventListener("pointermove",h),i.addEventListener("pointerup",g),i.addEventListener("pointercancel",b),i.addEventListener("keydown",I),()=>{i.removeEventListener("pointerdown",d),i.removeEventListener("pointermove",h),i.removeEventListener("pointerup",g),i.removeEventListener("pointercancel",b),i.removeEventListener("keydown",I)}}var R=(i="")=>`
...
`;var lt=new Set(["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"]),dt=new Set(["hs","rgb","rgbw","rgbww","xy"]),_e=i=>i?.attributes?.supported_color_modes??[],ht=i=>_e(i).some(e=>lt.has(e)),ne=i=>_e(i).some(e=>dt.has(e)),ie=i=>_e(i).includes("color_temp"),Re=i=>Math.max(1,Math.round((i??0)/255*100)),we=class extends v{static css=`
    :host { display: block; }
  ...
  `;validate(e){let t=e.entity??e.lights?.[0]??e.entities?.[0],n=typeof t=="string"?t:t?.entity;return n?{show_colour:!0,...e,entity:n}:{...e,[E]:"Kies een lamp."}}watched(){return[this.config.entity]}template(){return this.config.bare&&this.setAttribute("bare",""),`
      ...
      </div>`}wire(){let e=this.config.entity;this.teardown_.push(C(this.$(".chip"),{onTap:()=>this.hass.callService("light","toggle",{entity_id:e}),onHold:()=>N(this,e)})),this.$(".card").addEventListener("click",t=>{t.target.closest(".toggle")&&this.hass.callService("light","toggle",{entity_id:e})}),this.sliders_=new Map}attach_(e,t,n){if(!e||this.sliders_.has(t))return;let a=ee(e,n);this.sliders_.set(t,a),this.teardown_.push(a)}setSlider_(e,t,n=0,a=100){if(!e)return;let r=a>n?(t-n)/(a-n)*100:0;e.style.setProperty("--v",`${r}%`),e.setAttribute("aria-valuemin",String(n)),e.setAttribute("aria-valuemax",String(a)),e.setAttribute("aria-valuenow",String(t))}paint(){let e=this.config,t=m(this.hass,e.entity),n=D(t),a=t?.state==="on",r=this.$(".lamp");r.dataset.on=String(a),r.classList.toggle("unavailable",n);let s=this.$(".chip"),c=e.icon||"bulb";s.dataset.icon!==c&&(s.dataset.icon=c,s.innerHTML=w(c,"bulb")),this.text(".nm",M(this.hass,e.entity,e.name));let l=a?t?.attributes?.rgb_color:null;r.style.setProperty("--tone",l?`rgb(${l[0]},${l[1]},${l[2]})`:"var(--dac-lit)");let d=this.$(".ctl"),h=n?"none":ht(t)?"range":"toggle";if(d.dataset.kind!==h&&(d.dataset.kind=h,d.innerHTML=h==="range"?R("brightness"):h==="toggle"?'<button class="toggle" type="button" role="switch" aria-checked="false" aria-label="Aan of uit"></button>':"",this.sliders_.delete("brightness")),h==="range"){let g=d.querySelector(".slider");if(this.attach_(g,"brightness",{value:()=>t?.state==="on"?Re(m(this.hass,e.entity)?.attributes?.brightness):0,onInput:b=>{this.setSlider_(g,b),this.text(".v",b===0?"Uit":`${b}%`)},onCommit:b=>{b===0?this.hass.callService("light","turn_off",{entity_id:e.entity}):this.hass.callService("light","turn_on",{entity_id:e.entity,brightness_pct:b})},disabled:()=>D(m(this.hass,e.entity))}),!g.classList.contains("dragging")){let b=a?Re(t.attributes.brightness):0;this.setSlider_(g,b),this.text(".v",a?`${b}%`:"Uit")}}else h==="toggle"?(d.querySelector(".toggle")?.setAttribute("aria-checked",String(a)),this.text(".v",a?"Aan":"Uit")):this.text(".v","Niet bereikbaar");this.paintColour_(t,a)}paintColour_(e,t){let n=this.$(".colour"),a=this.config.show_colour!==!1&&(ne(e)||ie(e));if(n.hidden=!(a&&t),!a)return;let r=`${ne(e)?"c":""}${ie(e)?"t":""}`;if(n.dataset.sig!==r){n.dataset.sig=r,n.innerHTML=(ne(e)?`<span data-kind="hue" style="display:contents">${R("hue")}</span>`:"")+(ie(e)?`<span data-kind="kelvin" style="display:contents">${R("kelvin")}</span>`:"");let d=n.querySelector(".slider.hue");d&&(d.dataset.strip="",d.style.setProperty("--strip","linear-gradient(90deg, hsl(0 90% 55%), hsl(60 90% 55%), hsl(120 90% 55%), hsl(180 90% 55%), hsl(240 90% 55%), hsl(300 90% 55%), hsl(360 90% 55%))"),d.setAttribute("aria-label","Kleur"));let h=n.querySelector(".slider.kelvin");h&&(h.dataset.strip="",h.style.setProperty("--strip","linear-gradient(90deg,#ffb15e,#ffd6a8,#fff5e8,#eaf1ff,#cbdcff)"),h.setAttribute("aria-label","Kleurtemperatuur")),this.sliders_.delete("hue"),this.sliders_.delete("kelvin")}if(!t)return;let s=this.config.entity,c=n.querySelector(".slider.hue");c&&(this.attach_(c,"hue",{min:0,max:360,value:()=>m(this.hass,s)?.attributes?.hs_color?.[0]??0,onInput:d=>this.setSlider_(c,d,0,360),onCommit:d=>{let h=m(this.hass,s)?.attributes?.hs_color?.[1]??100;this.hass.callService("light","turn_on",{entity_id:s,hs_color:[d,h]})}}),c.classList.contains("dragging")||this.setSlider_(c,Math.round(e.attributes.hs_color?.[0]??0),0,360));let l=n.querySelector(".slider.kelvin");if(l){let d=e.attributes.min_color_temp_kelvin??2e3,h=e.attributes.max_color_temp_kelvin??6500;if(this.attach_(l,"kelvin",{min:d,max:h,step:50,value:()=>m(this.hass,s)?.attributes?.color_temp_kelvin??d,onInput:g=>this.setSlider_(l,g,d,h),onCommit:g=>this.hass.callService("light","turn_on",{entity_id:s,color_temp_kelvin:g})}),!l.classList.contains("dragging")){let g=e.attributes.color_temp_kelvin;g!=null&&this.setSlider_(l,g,d,h)}}}getCardSize(){let e=m(this.hass,this.config?.entity);return e?.state==="on"&&(ne(e)||ie(e))?2:1}getGridOptions(){return{columns:12,rows:"auto",min_columns:4,min_rows:1}}static getConfigElement(){return document.createElement("domotiapp-light-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("light."));return n?{entity:n}:{}}},ye=class extends x{defaults(){return{show_colour:!0}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"bulb"}]}schema(){return[{name:"entity",selector:p.entity("light")},{name:"name",selector:p.text()},{name:"show_colour",selector:p.bool()}]}label(e){return{entity:"Lamp",name:"Naam (overschrijft die van de lamp)",show_colour:"Kleurstrips tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"E\xE9n lamp per kaart. Dimbaar krijgt een schuif, alleen schakelbaar een tuimelaar.";if(e.name==="show_colour")return"Kleur en kleurtemperatuur verschijnen zodra de lamp aan is. De kaart is dan twee rijen hoog."}};_("domotiapp-light-card-editor",ye);y("domotiapp-light-card",we,{name:"DomotiApp Verlichting",description:"E\xE9n lamp op \xE9\xE9n rasterrij: dimmen, kleur en kleurtemperatuur."});function We(i){if(!i)return null;let e=Number(i.state);return Number.isFinite(e)?e:null}function pt(i){let e=i?.attributes?.hvac_action;return e||(i?.state==="off"?"off":i?.state==="cool"?"cooling":i?.state==="heat"?"idle":null)}var ke={heating:"var(--dac-solar)",cooling:"var(--dac-grid-in)",drying:"var(--dac-grid-in)",fan:"var(--dac-grid-in)"},Ke={heating:"Verwarmt",cooling:"Koelt",drying:"Ontvochtigt",fan:"Ventileert",idle:"Uit",off:"Uit"},Me=class extends v{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return e.entity||e.temperature||e.humidity?{...e}:{...e,[E]:"Kies een thermostaat, of een temperatuursensor."}}watched(){let e=this.config;return[e.entity,e.temperature,e.humidity].filter(Boolean)}step_(){let e=k(this.hass,this.config.entity);return Number(this.config.step??e.target_temp_step)||.5}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),e.entity||this.setAttribute("readout",""),`
      <div class="card surface">
        ...
        ${e.entity?`<div class="set">
                 <button type="button" data-d="-1" aria-label="Lager">${f.minus}</button>
                 <span class="target tnum"></span>
                 <button type="button" data-d="1" aria-label="Hoger">${f.plus}</button>
               </div>`:""}
      </div>`}wire(){let e=this.config;this.teardown_.push(C(this.$(".chip"),{onTap:()=>N(this,e.entity||e.temperature||e.humidity)}));let t=this.$(".set");t&&t.querySelectorAll("button").forEach(n=>n.addEventListener("click",()=>this.nudge_(Number(n.dataset.d))))}nudge_(e){let t=this.config,n=k(this.hass,t.entity),a=this.step_(),r=Number(n.min_temp??5),s=Number(n.max_temp??35),c=this.pending_??Number(n.temperature);if(!Number.isFinite(c))return;let l=Math.min(s,Math.max(r,Math.round((c+e*a)/a)*a));this.pending_=l,this.paintTarget_(),clearTimeout(this.sendTimer_),this.sendTimer_=setTimeout(()=>{this.hass.callService("climate","set_temperature",{entity_id:t.entity,temperature:this.pending_}),setTimeout(()=>{this.pending_=null,this.paint()},1500)},450),this.teardown_.push(()=>clearTimeout(this.sendTimer_))}paintTarget_(){let e=this.$(".target");if(!e)return;let t=k(this.hass,this.config.entity),n=this.pending_??Number(t.temperature);e.classList.toggle("pending",this.pending_!=null),e.textContent=Number.isFinite(n)?`${L(this.hass,n,n%1?1:0)}\xB0`:"--"}paint(){let e=this.config,t=e.entity?m(this.hass,e.entity):null,n=e.entity?D(t):!1;this.toggleAttribute("dead",n);let a=pt(t),r=e.tone?$(e.tone):ke[a]??"var(--dac-ink-3)";this.$(".card").style.setProperty("--tone",r),this.toggleAttribute("busy",!!ke[a]);let s=this.$(".chip"),c=e.icon||"thermo";s.dataset.icon!==c&&(s.dataset.icon=c,s.innerHTML=w(c,"thermo")),s.style.setProperty("--tone",ke[a]?r:"var(--dac-ink-3)"),this.text(".nm",M(this.hass,e.entity||e.temperature||e.humidity,e.name));let l=e.temperature?We(m(this.hass,e.temperature)):Number(k(this.hass,e.entity).current_temperature),d=this.hass?.config?.unit_system?.temperature??"\xB0C";this.text(".temp",Number.isFinite(l)?`${L(this.hass,l,1)} ${d}`:"--");let h=e.humidity?We(m(this.hass,e.humidity)):null,g=this.$(".hum");g.innerHTML=h==null?"":`${f.drop}${L(this.hass,h,0)}%`,this.text(".sep",h==null?"":"\xB7"),e.entity&&!e.humidity&&Ke[a]&&a!=="idle"&&(this.text(".sep","\xB7"),g.textContent=Ke[a]),this.paintTarget_();let b=this.$(".set");if(b){let I=k(this.hass,e.entity),u=this.pending_??Number(I.temperature);b.querySelector('[data-d="-1"]').disabled=n||u<=Number(I.min_temp??5),b.querySelector('[data-d="1"]').disabled=n||u>=Number(I.max_temp??35)}}getCardSize(){return 1}getGridOptions(){return{columns:12,rows:1,min_columns:4,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-climate-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("climate."));return n?{entity:n}:{}}},$e=class extends x{pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"thermo"},{key:"tone",kind:"tone",label:"Vaste kleur (leeg = volgt de ketel)"}]}schema(){return[{name:"entity",selector:p.entity("climate")},{name:"temperature",selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"humidity",selector:{entity:{domain:"sensor",device_class:"humidity"}}},{name:"name",selector:p.text()},{name:"step",selector:p.number(.1,5,.1)}]}label(e){return{entity:"Thermostaat (optioneel)",temperature:"Temperatuursensor (optioneel)",humidity:"Vochtigheidssensor (optioneel)",name:"Naam",step:"Stap van de knoppen"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"Leeg laten voor een kaart die alleen meet. Met thermostaat komen de stelknoppen erbij.";if(e.name==="temperature")return"Wint van de meting van de thermostaat zelf. Handig als er een betere sensor in de kamer hangt.";if(e.name==="step")return"Leeg laten volgt de thermostaat, en anders een halve graad."}};_("domotiapp-climate-card-editor",$e);y("domotiapp-climate-card",Me,{name:"DomotiApp Klimaat",description:"Thermostaat, losse temperatuur- en vochtsensor, of allebei."});var Ge=44,Ue=6,Se=class extends v{static css=`
    :host { display: block; height: 100%; }
    ...
    .it {
      display: flex; align-items: center; gap: 10px;
      min-height: ${Ge}px; padding: 2px 6px 2px 2px;
    ...
    /* Onder de 260px passen twee namen niet meer naast elkaar zonder te
       verminken, dus dan gaat het raster terug naar \xE9\xE9n kolom. */
    @container (max-width: 260px) {
      .grid { grid-template-columns: 1fr; }
    }
  `;validate(e){let t=e.items??e.entities??[];return t.length?{columns:2,show_state:!0,...e,items:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Voeg minstens \xE9\xE9n entiteit toe."}}watched(){return this.config.items.map(e=>e.entity)}tone_(e){if(e.tone)return $(e.tone);if(this.config.tone)return $(this.config.tone);if(A(e.entity)!=="light")return O.accent;let t=m(this.hass,e.entity),n=t?.state==="on"?t.attributes?.rgb_color:null;return n?`rgb(${n[0]},${n[1]},${n[2]})`:O.lit}template(){let e=this.config;e.bare&&this.setAttribute("bare",""),this.style.containerType="inline-size";let t=e.items.map((a,r)=>`
      <div class="it" role="button" tabindex="0" data-i="${r}">
        <span class="chip"></span>
        <span class="txt"><span class="nm"></span><span class="st"></span></span>
      </div>`).join("");return`<div class="card surface"><div class="grid" style="--cols:${Math.min(Math.max(1,Number(e.columns)||2),3)}">${t}</div></div>`}wire(){this.$$(".it").forEach(e=>{let t=this.config.items[+e.dataset.i],n=(a,r)=>J(this,this.hass,t,t[a]??r);this.teardown_.push(C(e,{onTap:()=>n("tap_action",Y(t.entity)),onHold:()=>n("hold_action",{action:"more-info"})}))})}paint(){this.$$(".it").forEach(e=>{let t=this.config.items[+e.dataset.i],n=m(this.hass,t.entity),a=X(n),r=D(n);e.dataset.on=String(a),e.classList.toggle("unavailable",r);let s=this.tone_(t);e.style.setProperty("--tone",s);let c=e.querySelector(".chip"),l=t.icon||Q(t.entity,k(this.hass,t.entity));c.dataset.icon!==l&&(c.dataset.icon=l,c.innerHTML=w(l)),c.style.setProperty("--tone",a?s:"var(--dac-ink-3)");let d=M(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),d);let h=e.querySelector(".st");if(this.config.show_state===!1)h.textContent="";else if(r)h.textContent="Niet bereikbaar";else if(!n||K(n.entity_id))h.textContent="";else if(A(n.entity_id)==="light"&&a&&n.attributes.brightness!=null)h.textContent=`${Math.round(n.attributes.brightness/255*100)}%`;else{let g=n.attributes.unit_of_measurement;h.textContent=g?`${n.state} ${g}`:T(this.hass,n)}e.setAttribute("aria-label",`${d}${n?`, ${T(this.hass,n)}`:""}`)})}lines_(){let e=Math.min(Math.max(1,Number(this.config?.columns)||2),3);return Math.ceil((this.config?.items?.length??1)/e)}getCardSize(){return this.lines_()}getGridOptions(){let e=this.lines_(),t=H(12+e*Ge+(e-1)*Ue);return{columns:12,rows:t,min_columns:4,min_rows:t,max_rows:t}}static getConfigElement(){return document.createElement("domotiapp-entities-card-editor")}static getStubConfig(e,t){return{entities:(t??[]).slice(0,2),columns:2}}},Ee=class extends x{defaults(){return{columns:2,show_state:!0}}setConfig(e){let t={...e},n=(e.items??e.entities??[]).map(a=>typeof a=="string"?{entity:a}:a);t.entities=n.map(a=>a.entity),delete t.items;for(let a of n)a.name&&(t[`naam:${a.entity}`]=a.name),a.icon&&(t[`icoon:${a.entity}`]=a.icon),a.tone&&(t[`kleur:${a.entity}`]=a.tone);super.setConfig(t)}serialize(e){let t={...e},n=t.entities??[];t.items=n.map(a=>{let r={entity:a};return t[`naam:${a}`]&&(r.name=t[`naam:${a}`]),t[`icoon:${a}`]&&(r.icon=t[`icoon:${a}`]),t[`kleur:${a}`]&&(r.tone=t[`kleur:${a}`]),r}),delete t.entities;for(let a of Object.keys(t))/^(naam|icoon|kleur):/.test(a)&&delete t[a];return t}pickers(){return(this.config_?.entities??[]).filter(t=>typeof t=="string").flatMap(t=>[{key:`icoon:${t}`,kind:"icon",label:`Icoon voor ${this.short_(t)}`,fallback:"star"},{key:`kleur:${t}`,kind:"tone",label:`Kleur voor ${this.short_(t)}`}])}short_(e){return this.hass?.states?.[e]?.attributes?.friendly_name??e}schema(){let e=(this.config_?.entities??[]).filter(t=>typeof t=="string");return[{name:"entities",selector:{entity:{multiple:!0}}},{name:"columns",selector:p.select([{value:1,label:"1 per rij"},{value:2,label:"2 naast elkaar"},{value:3,label:"3 naast elkaar"}])},{name:"show_state",selector:p.bool()},...e.map(t=>({name:`naam:${t}`,selector:p.text()}))]}label(e){return e.name.startsWith("naam:")?`Naam voor ${this.short_(e.name.slice(5))}`:{entities:"Entiteiten",columns:"Kolommen",show_state:"Toestand tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="entities")return"Per entiteit kun je hieronder een eigen icoon, kleur en naam zetten.";if(e.name==="columns")return"Onder de 260 pixels valt de kaart vanzelf terug op \xE9\xE9n kolom."}};_("domotiapp-entities-card-editor",Ee);y("domotiapp-entities-card",Se,{name:"DomotiApp Entiteiten",description:"Een lijst entiteiten, per \xE9\xE9n, twee of drie naast elkaar."});var U={OPEN:1,CLOSE:2,SET_POSITION:4,STOP:8},B=(i,e)=>!!((i?.attributes?.supported_features??0)&e),ut=(i={})=>{switch(i.device_class){case"garage":return{open:"garageOpen",closed:"garageClosed"};case"awning":case"blind":return{open:"awning",closed:"awning"};default:return{open:"shutterOpen",closed:"shutter"}}},ze=class extends v{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.covers??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,covers:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Kies minstens \xE9\xE9n rolluik of zonnescherm."}}watched(){return this.config.covers.map(e=>e.entity)}keysHtml(e){return`
      <div class="keys">
      ...
      </div>`).join("")}</div>`}wire(){this.dragging_=new Set,this.assumed_=new Map,this.$$(".cv").forEach(e=>{let t=e.dataset.i;e.querySelectorAll(".keys button").forEach(a=>{a.addEventListener("click",()=>{let r=a.dataset.act,s={open:"open_cover",stop:"stop_cover",close:"close_cover"};this.hass.callService("cover",s[r],{entity_id:this.config.covers[+t].entity}),r!=="stop"&&(this.assumed_.set(t,r==="open"?"open":"closed"),this.paint())})});let n=this.config.covers[+t].entity;this.teardown_.push(C(e.querySelector(".chip"),{onTap:()=>N(this,n)}))})}paint(){this.$$(".cv").forEach(e=>{let t=e.dataset.i,n=this.config.covers[+t],a=m(this.hass,n.entity),r=k(this.hass,n.entity),s=!a||a.state==="unavailable",c=a?.state??"unknown";e.classList.toggle("unavailable",s),e.querySelector(".nm").textContent=M(this.hass,n.entity,n.name);let l=B(a,U.SET_POSITION)&&r.current_position!=null,d=l?r.current_position>0?"open":"closed":c==="open"||c==="closed"?c:this.assumed_.get(t)??"closed";e.dataset.shown=d;let h=ut(r),g=(d==="open"?n.icon_open:n.icon_closed)??(d==="open"?this.config.icon_open:this.config.icon_closed)??n.icon??h[d],b=e.querySelector(".chip");b.dataset.icon!==g&&(b.dataset.icon=g,b.innerHTML=w(g,h[d]));let I=e.querySelector(".st");this.dragging_.has(t)||(I.textContent=s?"Niet bereikbaar":c==="opening"?"Gaat open":c==="closing"?"Gaat dicht":l?`${r.current_position}% open`:c==="open"?"Open":c==="closed"?"Dicht":""),e.querySelectorAll(".keys button").forEach(S=>{if(S.dataset.act==="stop"){S.disabled=s||!B(a,U.STOP);return}let z=S.dataset.act==="open";S.disabled=s||(z?!B(a,U.OPEN):!B(a,U.CLOSE))});let u=e.querySelector(".pos"),j=l&&this.config.show_position!==!1;if(u.hidden=!j,j){if(!u.dataset.built){u.dataset.built="1",u.innerHTML=R("position");let z=u.querySelector(".slider");z.setAttribute("aria-label","Positie");let F=W=>{z.style.setProperty("--v",`${W}%`),z.setAttribute("aria-valuenow",String(W)),e.querySelector(".st").textContent=`${W}% open`};this.teardown_.push(ee(z,{value:()=>k(this.hass,n.entity).current_position??0,onInput:F,onCommit:W=>this.hass.callService("cover","set_cover_position",{entity_id:n.entity,position:W})}))}let S=u.querySelector(".slider");if(!S.classList.contains("dragging")){let z=r.current_position??0;S.style.setProperty("--v",`${z}%`),S.setAttribute("aria-valuenow",String(z))}}})}rows_(){let e=this.config?.covers??[],t=e.some(n=>B(m(this.hass,n.entity),U.SET_POSITION));return H(12+Math.max(1,e.length)*42+(t?30:0))}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-cover-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("cover."));return{covers:n?[n]:[]}}},Ce=class extends x{defaults(){return{show_stop:!0,show_position:!0}}pickers(){return[{key:"icon_open",kind:"icon",label:"Icoon als het open staat",fallback:"shutterOpen"},{key:"icon_closed",kind:"icon",label:"Icoon als het dicht is",fallback:"shutter"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"covers",selector:{entity:{domain:"cover",multiple:!0}}},{name:"show_stop",selector:p.bool()}]}label(e){return{covers:"Rolluiken",show_stop:"Stopknop tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="covers")return"Melden ze hun stand terug, dan komt er vanzelf een schuif bij. Zo niet, dan blijven het open, stop en dicht, en volgt het icoon de knop die je indrukt."}};_("domotiapp-cover-card-editor",Ce);y("domotiapp-cover-card",ze,{name:"DomotiApp Rolluiken",description:"Open, stop en dicht, met een eigen icoon voor open en dicht."});function mt(i){if(!i)return{label:"Onbekend",home:null};switch(i.state){case"home":return{label:"Thuis",home:!0};case"not_home":return{label:"Afwezig",home:!1};case"unknown":case"unavailable":return{label:"Onbekend",home:null};default:return{label:i.state,home:!1}}}var Oe=class extends v{static css=`
    :host { display: block; height: 100%; }
    ...
    :focus-visible { outline: 2px solid var(--dac-accent-hi); outline-offset: 2px; }
  `;validate(e){let t=e.persons??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,persons:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Kies minstens \xE9\xE9n persoon."}}watched(){return this.config.persons.map(e=>e.entity)}template(){let e=this.config;e.bare&&this.setAttribute("bare","");let t=e.columns??Math.min(e.persons.length,6),n=e.persons.map((a,r)=>`
      <button class="p" type="button" data-i="${r}" style="--tone:var(--dac-ink-3)">
        <span class="av"><span class="ph"></span></span>
        <span class="nm"></span>
        <span class="st"></span>
      </button>`).join("");return`<div class="card surface"><div class="chips" style="--cols:${t}">${n}</div></div>`}wire(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i];this.teardown_.push(C(e,{onTap:()=>N(this,t.entity)}))})}paint(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i],n=m(this.hass,t.entity),a=mt(n);e.style.setProperty("--tone",a.home===!0?"var(--dac-good)":a.home===!1?"var(--dac-bad)":"var(--dac-warn)");let r=M(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),r),this.text(e.querySelector(".st"),a.label);let s=e.querySelector(".ph"),c=n?.attributes?.entity_picture,l=c?`img:${c}`:r?`ini:${r[0]}`:"icon";s.dataset.kind!==l&&(s.dataset.kind=l,s.innerHTML=c?`<img src="${c}" alt="" loading="lazy" />`:r?r[0].toUpperCase():f.person),e.setAttribute("aria-label",`${r}, ${a.label}`)})}rows_(){let e=this.config?.columns??Math.min(this.config?.persons?.length??1,6),t=Math.ceil((this.config?.persons?.length??1)/e);return H(20+t*74+(t-1)*6)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:"full",rows:e,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-person-card-editor")}static getStubConfig(e){return{persons:Object.keys(e?.states??{}).filter(n=>n.startsWith("person.")).slice(0,6)}}},Ae=class extends x{setConfig(e){let t={...e},n=(e.persons??[]).map(a=>typeof a=="string"?{entity:a}:a);t.persons=n.map(a=>a.entity);for(let a of n)a.name&&(t[`naam:${a.entity}`]=a.name);super.setConfig(t)}serialize(e){let t={...e},n=t.persons??[];t.persons=n.map(a=>{let r=t[`naam:${a}`];return r?{entity:a,name:r}:a});for(let a of Object.keys(t))a.startsWith("naam:")&&delete t[a];return t}schema(){let e=(this.config_?.persons??[]).filter(t=>typeof t=="string");return[{name:"persons",selector:{entity:{domain:["person","device_tracker"],multiple:!0}}},...e.map(t=>({name:`naam:${t}`,selector:p.text()}))]}label(e){if(e.name==="persons")return"Personen";if(e.name.startsWith("naam:")){let t=e.name.slice(5);return`Naam voor ${this.hass?.states?.[t]?.attributes?.friendly_name??t}`}return super.label(e)}helper(e){if(e.name==="persons")return"Thuis is groen, weg is rood, geen melding is oranje. Per persoon kun je hieronder een eigen naam zetten."}};_("domotiapp-person-card-editor",Ae);y("domotiapp-person-card",Oe,{name:"DomotiApp Personen",description:"Wie er thuis is, compact. Het hele huishouden in \xE9\xE9n kaart."});var gt=[[/gft|groente|tuin|organi/i,"teal","binWheeled"],[/pmd|plastic|verpakking/i,"solar","binWheeled"],[/papier|karton/i,"water","binWheeled"],[/rest|grijs/i,"neutral","binWheeled"],[/textiel|kleding/i,"pink","bin"],[/glas/i,"magenta","bin"],[/kerstboom|snoei|takken/i,"teal","bin"]];function ft(i){for(let[e,t,n]of gt)if(e.test(i))return{tone:t,icon:n};return{tone:"accent",icon:"bin"}}var Be=i=>String(i??"").replace(/^(afvalbeheer|afvalwijzer|mijnafvalwijzer)\s*/i,"").replace(/\s*(mijnafvalwijzer)\s*/i," ").trim(),Le=class extends v{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.sensors??e.entities??(e.entity?[e.entity]:[]);return t.length?{show_hero:!0,show_list:!0,...e,sensors:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[E]:"Kies minstens \xE9\xE9n afvalsensor waarvan de toestand een datum is."}}watched(){return this.config.sensors.map(e=>e.entity)}read_(){let e=new Date;return this.config.sensors.map(t=>{let n=m(this.hass,t.entity);if(!n)return null;let a=G(n.state)??G(n.attributes.date)??G(n.attributes.next_date);if(!a)return null;let r=t.label??Be(M(this.hass,t.entity,t.name)),s=ft(t.label??t.entity+r),c=this.config.tones?.[t.entity];return{label:r,date:a,days:ae(e,a),tone:$(c??t.tone??s.tone),icon:t.icon??s.icon}}).filter(t=>t&&t.days>=0).sort((t,n)=>t.date-n.date)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),`
      <div class="card surface">
      ...
      </div>`}paint(){let e=this.read_(),t=this.$(".hero"),n=this.$(".list"),a=this.$(".empty");if(a.hidden=e.length>0,t&&(t.hidden=e.length===0,e.length)){let r=e[0];t.style.setProperty("--tone",r.tone),this.setAttribute("urgency",r.days===0?"today":r.days===1?"tomorrow":"later");let s=t.querySelector(".bin");s.dataset.icon!==r.icon&&(s.dataset.icon=r.icon,s.innerHTML=w(r.icon,"bin")),this.text(t.querySelector(".eyebrow"),re(r.date)),this.text(t.querySelector(".big"),r.label),this.text(t.querySelector(".n"),r.days===0?"nu":String(r.days)),this.text(t.querySelector(".u"),r.days===0?"aan de weg":r.days===1?"dag":"dagen")}if(n){let r=this.config.show_hero===!1?e:e.slice(1),s=r.map(c=>`${c.label}${+c.date}`).join("|");if(n.dataset.sig===s)return;n.dataset.sig=s,n.innerHTML=r.map(c=>{let l=re(c.date),d=c.days<=6?`<small>${Pe(c.date)}</small>`:"";return`
        <div class="r" style="--tone:${c.tone}">
          <i></i><span>${c.label}</span>
          <span class="d">${l}${d}</span>
        </div>`}).join("")}}rows_(){let e=this.config?.sensors?.length??1;return this.config?.show_list===!1?1:this.config?.show_hero===!1?Math.max(1,H(20+e*33)):Math.max(2,e)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-waste-card-editor")}static getStubConfig(e){return{sensors:Object.keys(e?.states??{}).filter(n=>/afval|waste|trash|garbage|ophaal/i.test(n)&&n.startsWith("sensor.")).filter(n=>G(e.states[n]?.state)).slice(0,6),title:"Afvalkalender"}}},Te=class extends x{defaults(){return{show_hero:!0,show_list:!0}}setConfig(e){let t={...e};for(let[n,a]of Object.entries(e.tones??{}))t[`kleur:${n}`]=a;delete t.tones,super.setConfig(t)}serialize(e){let t={...e},n={};for(let a of Object.keys(t))a.startsWith("kleur:")&&(t[a]&&(n[a.slice(6)]=t[a]),delete t[a]);return Object.keys(n).length?t.tones=n:delete t.tones,t}schema(){let e=this.config_?.sensors??[],t=Object.keys(O).map(n=>({value:n,label:V[n]??n}));return[{name:"sensors",selector:{entity:{domain:"sensor",multiple:!0}}},...e.map(n=>typeof n=="string"?n:n.entity).filter(Boolean).map(n=>({name:`kleur:${n}`,selector:p.select(t)}))]}label(e){if(e.name.startsWith("kleur:")){let t=e.name.slice(6),n=this.hass?.states?.[t]?.attributes?.friendly_name??t;return`Kleur voor ${Be(n)||n}`}return{sensors:"Afvalsensoren",show_hero:"Eerstvolgende uitlichten",show_list:"Overige data tonen",bare:"Zonder kaartrand"}[e.name]??super.label(e)}helper(e){if(e.name==="sensors")return"Sensoren waarvan de toestand een datum is, bijvoorbeeld 18-08-2026. De kaart sorteert zelf; laat een kleur leeg om de bakkleur op de naam te laten kiezen."}};_("domotiapp-waste-card-editor",Te);y("domotiapp-waste-card",Le,{name:"DomotiApp Afvalkalender",description:"Eerstvolgende ophaling als hero, de rest eronder. Kleur per fractie."});var bt="0.1.6";console.info(`%c DOMOTIAPP-CARDS %c ${bt} `,"background:#026fa1;color:#e8e4de;font-weight:600;border-radius:3px 0 0 3px;padding:2px 6px","background:#12120f;color:#e8e4de;border-radius:0 3px 3px 0;padding:2px 6px");export{bt as VERSION};
