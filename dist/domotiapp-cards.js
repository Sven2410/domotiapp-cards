/*! DomotiApp Cards 0.1.11 | MIT | https://github.com/Sven2410/domotiapp-cards */
var R=`
  --dac-bg:            #0c0c0a;
  --dac-bg-raise:      #12120f;
...
`;function V(i){let e=new CSSStyleSheet;return e.replaceSync(i),e}var L=i=>String(i??"").split(".")[0],f=(i,e)=>e&&i?.states?.[e]||null,M=(i,e)=>f(i,e)?.attributes??{},J=(i,e,t)=>t?null:M(i,e).entity_picture||null;function Q(i){if(!i||i.state!=="on")return null;let e=i.attributes??{};if(Array.isArray(e.entity_id))return null;let t=e.rgb_color;return Array.isArray(t)&&t.length>=3?`rgb(${t[0]},${t[1]},${t[2]})`:null}function z(i,e,t){return t||M(i,e).friendly_name||e||""}var it=new Set(["scene","script","input_button","button","event"]),B=i=>it.has(L(i));function I(i){return!i||i.state==="unavailable"?!0:i.state==="unknown"?!B(i.entity_id):!1}function ee(i){if(!i)return!1;let e=i.state;if(e==="unavailable"||e==="unknown")return!1;switch(L(i.entity_id)){case"cover":return e==="open"||e==="opening";case"alarm_control_panel":return e.startsWith("armed")||e==="triggered"||e==="arming";case"climate":case"water_heater":case"humidifier":return e!=="off";case"person":case"device_tracker":return e==="home";case"media_player":return e!=="off"&&e!=="idle"&&e!=="standby";default:return e==="on"||e==="playing"||e==="active"||e==="heat"}}function We(i,e,t){if(!i||i.themes!==e.themes||i.language!==e.language)return!0;for(let n of t)if(n&&i.states?.[n]!==e.states?.[n])return!0;return!1}function Y(i,e,t={}){i.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1}))}var P=(i,e)=>Y(i,"hass-more-info",{entityId:e});function te(i){switch(L(i)){case"light":case"switch":case"fan":case"input_boolean":case"automation":case"siren":return{action:"toggle"};case"script":case"scene":case"input_button":case"button":return{action:"toggle"};default:return{action:"more-info"}}}function at(i){switch(L(i)){case"scene":return["scene","turn_on"];case"script":return["script","turn_on"];case"input_button":return["input_button","press"];case"button":return["button","press"];case"lock":return["lock","open"];case"cover":return["cover","toggle"];case"media_player":return["media_player","media_play_pause"];default:return["homeassistant","toggle"]}}function ne(i,e,t,n){if(!(!n||n.action==="none"))switch(n.action){case"more-info":P(i,n.entity||t.entity);break;case"toggle":{let a=n.entity||t.entity;if(!a)break;let[o,r]=at(a);e.callService(o,r,{entity_id:a});break}case"perform-action":case"call-service":{let a=n.perform_action||n.service;if(!a)break;let[o,r]=a.split(".");e.callService(o,r,n.data??n.service_data??{},n.target);break}case"navigate":if(!n.navigation_path)break;history.pushState(null,"",n.navigation_path),Y(window,"location-changed",{replace:!1});break;case"url":n.url_path&&window.open(n.url_path,n.target??"_blank");break;case"assist":Y(i,"show-dialog",{dialogTag:"ha-voice-command-dialog",dialogImport:()=>{},dialogParams:{}});break;case"fire-dom-event":Y(i,"ll-custom",n);break;default:break}}function A(i,{onTap:e,onHold:t,onDouble:n}){let r=0,s=0,l=null,h=u=>{u.button!=null&&u.button!==0||(r=Date.now())},p=()=>{let u=r?Date.now()-r:0;if(r=0,t&&u>=500){navigator.vibrate?.(18),t();return}if(!n){e?.();return}if(s++,s===1){l=setTimeout(()=>{s=0,e?.()},260);return}clearTimeout(l),s=0,n()};return i.addEventListener("pointerdown",h),i.addEventListener("click",p),i.addEventListener("contextmenu",u=>u.preventDefault()),()=>{clearTimeout(l),i.removeEventListener("pointerdown",h),i.removeEventListener("click",p)}}function D(i,e){if(!e)return"";let t=L(e.entity_id),n=e.attributes.device_class;return i.formatEntityState?.(e)??i.localize?.(`component.${t}.entity_component.${n??"_"}.state.${e.state}`)??i.localize?.(`component.${t}.entity_component._.state.${e.state}`)??e.state}function N(i,e,t){let n=Number(e);return Number.isFinite(n)?n.toLocaleString(i?.locale?.language??"nl",{minimumFractionDigits:t??0,maximumFractionDigits:t??0}):"--"}var Re=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],Ke=["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"],Ve=(i=new Date)=>new Date(i.getFullYear(),i.getMonth(),i.getDate()),le=(i,e)=>Math.round((Ve(e)-Ve(i))/864e5);function G(i){if(!i)return null;if(i instanceof Date)return Number.isNaN(+i)?null:i;let e=String(i).trim(),t=e.match(/^(\d{1,2})-(\d{1,2})-(\d{4})/);if(t)return new Date(+t[3],+t[2]-1,+t[1]);if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/),t)return new Date(+t[1],+t[2]-1,+t[3]);let n=new Date(e);return Number.isNaN(+n)?null:n}function de(i,e=new Date){if(!i)return"";let t=le(e,i);return t<0?`${Math.abs(t)} dagen geleden`:t===0?"vandaag":t===1?"morgen":t===2?"overmorgen":t<=6?Re[i.getDay()]:`${Re[i.getDay()].slice(0,2)} ${i.getDate()} ${Ke[i.getMonth()]}`}var Be=i=>i?`${i.getDate()} ${Ke[i.getMonth()]}`:"";var ot=`
  :host {
    ${R}
    display: block;
...
`,T={accent:"var(--dac-accent-hi)",solar:"var(--dac-solar)",house:"var(--dac-house)",water:"var(--dac-grid-in)",magenta:"var(--dac-grid-out)",pink:"var(--dac-device-1)",teal:"var(--dac-device-2)",lit:"var(--dac-lit)",good:"var(--dac-good)",warn:"var(--dac-warn)",bad:"var(--dac-bad)",neutral:"var(--dac-ink-3)"},ie={accent:"Accent",solar:"Oranje",house:"Blauw",water:"Lichtblauw",magenta:"Magenta",pink:"Roze",teal:"Groenblauw",lit:"Lampgeel",good:"Goed",warn:"Let op",bad:"Kritiek",neutral:"Neutraal"},C=(i,e="accent")=>T[i]??(i&&/[#(]|^var/.test(i)?i:T[e]),O=Symbol("incomplete"),rt=i=>`
  <div class="needs">
  ...
  </div>`,st=56,Ge=8,H=i=>Math.max(1,Math.ceil((i+Ge)/(st+Ge))),_=class extends HTMLElement{static css="";static get styleSheets_(){return Object.hasOwn(this,"sheets_")||(this.sheets_=[V(ot+qe+this.css)]),this.sheets_}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=new.target.styleSheets_,this.built_=!1,this.wired_=!1,this.teardown_=[]}setConfig(e){let t=this.validate(e??{});this.config=t,this.built_&&(this.destroy_(),this.shadowRoot.replaceChildren(),this.built_=!1,this.wired_=!1),this.isConnected&&this.build_()}set hass(e){let t=this.hass_;if(this.hass_=e,!!this.config){if(!this.built_){this.build_();return}this.config[O]||We(t,e,this.watched())&&this.paint()}}get hass(){return this.hass_}connectedCallback(){if(this.config){if(!this.built_){this.build_();return}this.config[O]||this.wired_||(this.wire(),this.wired_=!0,this.hass_&&this.paint())}}disconnectedCallback(){this.destroy_(),this.wired_=!1}validate(e){return e}watched(){return this.config?.entity?[this.config.entity]:[]}template(){return""}wire(){}paint(){}build_(){let e=document.createElement("template"),t=this.config?.[O];e.innerHTML=t?rt(t):this.template(),this.shadowRoot.appendChild(e.content),this.built_=!0,!t&&(this.wire(),this.wired_=!0,this.hass_&&this.paint())}destroy_(){for(let e of this.teardown_)try{e()}catch{}this.teardown_=[]}on(e,t,n,a){e&&(e.addEventListener(t,n,a),this.teardown_.push(()=>e.removeEventListener(t,n,a)))}$(e){return this.shadowRoot.querySelector(e)}$$(e){return[...this.shadowRoot.querySelectorAll(e)]}text(e,t){let n=typeof e=="string"?this.$(e):e;n&&n.textContent!==String(t)&&(n.textContent=t)}getCardSize(){return 1}};function S(i,e,{name:t,description:n,preview:a=!0}={}){customElements.get(i)||(customElements.define(i,e),window.customCards=window.customCards??[],window.customCards.push({type:i,name:t??i,description:n??"",preview:a,documentationURL:"https://github.com/Sven2410/domotiapp-cards"}))}function j(i,e){customElements.get(i)||customElements.define(i,e)}var c=(i,e="none")=>`<svg class="icon" viewBox="0 0 24 24" fill="${e}" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${i}</svg>`,x={house:c(`<path d="M3.2 11.3 12 4.1l8.8 7.2"/>
    ...
    <path d="M9.6 9.6a2.4 2.4 0 1 1 3.2 2.3c-.5.2-.8.7-.8 1.2v.6M12 16.6v.1"/>`)};function $(i,e="question"){return i?x[i]?x[i]:i.includes(":")?`<ha-icon class="icon" icon="${i}"></ha-icon>`:x[e]??x.question:x[e]??x.question}function U(i,e={}){switch(String(i??"").split(".")[0]){case"light":return"bulb";case"switch":return"switchOn";case"cover":return e.device_class==="awning"||e.device_class==="blind"?"awning":"shutter";case"person":case"device_tracker":return"person";case"climate":return"thermo";case"binary_sensor":return e.device_class==="smoke"?"smoke":"shield";case"alarm_control_panel":return"shield";case"scene":case"script":case"input_button":case"button":return"star";case"weather":return"cloudSun";default:return"question"}}function Ue(i){switch(i){case"sunny":case"clear-night":return"sun";case"partlycloudy":return"cloudSun";case"cloudy":return"cloud";case"rainy":case"pouring":case"hail":case"lightning":case"lightning-rainy":return"rain";case"snowy":case"snowy-rainy":return"snow";case"fog":return"fog";case"windy":case"windy-variant":return"wind";default:return"cloud"}}var ct=[["Woning",["house","floorB","floor1","floor2","garage","door","window","grid"]],["Rolluiken",["shutter","shutterOpen","awning","garageOpen","garageClosed","arrowUp","arrowDown","stop"]],["Licht en stroom",["bulb","bulbGroup","switchOn","power","plug","bolt","battery"]],["Personen",["person","people","away"]],["Apparaten",["tv","speaker","camera","car","washer","dishwasher","printer","fan","airco","radio"]],["Afval",["bin","binWheeled","calendar"]],["Weer",["sun","cloud","cloudSun","rain","snow","fog","wind","drop","uv","sunrise","sunset","thermo"]],["Status",["shield","lock","lockOpen","key","wifi","smoke","warning","check","close","clock","gaugeArrow"]],["Overig",["star","moon","leaf","cog","dots","plus","minus","chevronRight","chevronDown","question"]]],lt=`
  :host { ${R} display: block; font-family: var(--dac-font); }
  *, *::before, *::after { box-sizing: border-box; }
...
`,he=null,pe=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),he=he??[V(lt)],this.shadowRoot.adoptedStyleSheets=he,this.value_="",this.label="Icoon",this.fallback="question",this.auto=!0}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}connectedCallback(){this.built_||(this.built_=!0,this.build_())}build_(){this.shadowRoot.innerHTML=`
        ...
        <div class="panel">
          ${ct.map(([t,n])=>`
            <div class="group">
              <h4>${t}</h4>
              <div class="grid">
                ${n.map(a=>`<button type="button" class="opt" data-icon="${a}" title="${a}" aria-pressed="false">${x[a]??""}</button>`).join("")}
              </div>
            </div>`).join("")}
          <div class="mdi">
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  :focus-visible { outline: 2px solid var(--dac-accent-hi); outline-offset: 2px; }
`,ue=null,me=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),ue=ue??[V(ut)],this.shadowRoot.adoptedStyleSheets=ue,this.value_="",this.label="Kleur",this.statuses=!1}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}set compact(e){this.toggleAttribute("compact",!!e)}get compact(){return this.hasAttribute("compact")}connectedCallback(){this.built_||(this.built_=!0,this.build_())}eigen_(){return!!this.value_&&!(this.value_ in T)}swatch(e){return`<button type="button" class="sw" data-tone="${e}" style="--c:${T[e]}"
      title="${ie[e]}" aria-label="${ie[e]}" aria-pressed="false">${x.check}</button>`}build_(){this.shadowRoot.innerHTML=`
      <div class="label"></div>
      <div class="box">
        ${this.statuses?"<h4>Identiteit</h4>":""}
        <div class="row">
          <button type="button" class="sw auto" data-tone="" title="Automatisch"
            aria-label="Automatisch" aria-pressed="false">${x.check}</button>
          ${dt.map(n=>this.swatch(n)).join("")}
        ...
        ${this.statuses?`<h4>Status</h4>
               <div class="row">${ht.map(n=>this.swatch(n)).join("")}</div>
               <p class="note">
      ...
      </div>`,this.shadowRoot.querySelectorAll("button.sw").forEach(n=>n.addEventListener("click",()=>this.emit_(n.dataset.tone)));let e=this.$('input[type="color"]');e.addEventListener("input",()=>this.emit_(e.value));let t=this.$("#vrij");t.addEventListener("change",()=>{let n=t.value.trim();if(!n){this.emit_("");return}let a=pt.test(n);t.setAttribute("aria-invalid",String(!a)),a&&this.emit_(n)}),this.$(".wissen").addEventListener("click",()=>this.emit_("")),this.paint_()}paint_(){if(!this.shadowRoot.firstElementChild)return;this.$(".label").textContent=this.label??"Kleur";let e=this.eigen_();this.shadowRoot.querySelectorAll("button.sw").forEach(a=>a.setAttribute("aria-pressed",String((a.dataset.tone||"")===this.value_)));let t=this.$(".eigen");t.setAttribute("aria-pressed",String(e)),t.classList.toggle("leeg",!e),t.style.setProperty("--c",e?this.value_:"transparent"),t.title=e?`Eigen kleur: ${this.value_}`:"Eigen kleur",e&&/^#[0-9a-f]{6}$/i.test(this.value_)&&(this.$('input[type="color"]').value=this.value_);let n=this.$("#vrij");if(this.shadowRoot.activeElement!==n){let a=e?this.value_:"";n.value!==a&&(n.value=a),n.setAttribute("aria-invalid","false")}this.$(".chosen").innerHTML=this.value_?e?`Gekozen: <b>${this.value_}</b> &mdash; eigen kleur.`:`Gekozen: <b>${ie[this.value_]??this.value_}</b>`:"Gekozen: <b>Automatisch</b> &mdash; de kaart kiest op domein en toestand."}emit_(e){this.value_=e??"",this.paint_(),this.dispatchEvent(new CustomEvent("value-changed",{detail:{value:this.value_},bubbles:!0,composed:!0}))}$(e){return this.shadowRoot.querySelector(e)}};customElements.get("dac-tone-picker")||customElements.define("dac-tone-picker",me);var g={entity:i=>({entity:i?{domain:i}:{}}),text:()=>({text:{}}),multiline:()=>({text:{multiline:!0}}),bool:()=>({boolean:{}}),number:(i,e,t=1)=>({number:{min:i,max:e,step:t,mode:"box"}}),select:i=>({select:{mode:"dropdown",options:i}}),action:(i="more-info")=>({ui_action:{default_action:i}})},ge=(...i)=>({type:"grid",name:"",schema:i});var E=class extends HTMLElement{constructor(){super(),this.config_={},this.built_=!1}setConfig(e){this.config_={...this.defaults(),...e},this.render_()}defaults(){return{}}set hass(e){this.hass_=e,this.form_&&(this.form_.hass=e);for(let t of this.pickers_??[])t.hass=e;this.render_()}get hass(){return this.hass_}connectedCallback(){this.render_()}schema(){return[]}pickers(){return[]}label(e){return fe[e.name]??e.name}helper(){}async render_(){if(!this.hass_||!this.config_)return;if(this.built_){this.sync_();return}this.built_=!0,await customElements.whenDefined("ha-form"),this.replaceChildren(),this.pickers_=[];let e=this.pickers();this.pickerSig_=e.map(r=>r.key).join("|");let t=r=>{let s=document.createElement("div");return s.style.cssText=`display:flex;flex-direction:column;gap:12px;${r}`,s},n=t("margin-bottom:16px"),a=t("margin-top:16px");for(let r of e){let s=document.createElement(r.kind==="tone"?"dac-tone-picker":"dac-icon-picker");s.label=r.label,s.fallback=r.fallback,r.auto===!1&&(s.auto=!1),r.statuses===!1&&(s.statuses=!1),r.compact&&(s.compact=!0),s.hass=this.hass_,s.value=this.config_[r.key],s.addEventListener("value-changed",l=>{l.stopPropagation(),this.patch_({[r.key]:l.detail.value})}),this.pickers_.push(s),s.dataset.key=r.key,(r.after?a:n).appendChild(s)}n.children.length&&this.appendChild(n);let o=document.createElement("ha-form");o.hass=this.hass_,o.data=this.config_,o.schema=this.schema(),o.computeLabel=r=>this.label(r),o.computeHelper=r=>this.helper(r),o.addEventListener("value-changed",r=>{r.stopPropagation(),this.patch_(r.detail.value,!0)}),this.form_=o,this.appendChild(o),a.children.length&&this.appendChild(a)}sync_(){let e=this.pickers().map(t=>t.key).join("|");if(this.pickerSig_!==void 0&&this.pickerSig_!==e){this.built_=!1,this.form_=null,this.render_();return}this.form_&&(this.form_.hass=this.hass_,this.form_.schema=this.schema(),this.form_.data=this.config_);for(let t of this.pickers_??[])t.hass=this.hass_,t.value=this.config_[t.dataset.key]}patch_(e,t=!1){let n=t?{...e}:{...this.config_,...e};this.config_.type&&(n.type=this.config_.type);for(let[a,o]of Object.entries(n))(o===""||o===void 0||o===null)&&delete n[a];this.config_=n,this.sync_(),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this.serialize(n)},bubbles:!0,composed:!0}))}serialize(e){return e}},fe={entity:"Entiteit",entities:"Entiteiten",name:"Naam",icon:"Icoon",tone:"Kleur",secondary:"Tweede regel",layout:"Vorm",tap_action:"Tikken",hold_action:"Vasthouden",double_tap_action:"Dubbeltikken",show_state:"Status tonen",show_name:"Naam tonen",show_icon:"Icoon tonen",fill:"Vullen",collapsible:"Inklapbaar",title:"Titel",subtitle:"Ondertitel",weather:"Weerentiteit",sun:"Zon-entiteit",person:"Persoon",persons:"Personen",covers:"Rolluiken",lights:"Lampen",sensors:"Sensoren",greeting:"Begroeting",show_clock:"Klok tonen",show_weather:"Weer tonen",show_chips:"Weerdetails tonen",compact:"Compact",columns:"Kolommen",group:"Groepsregel tonen",invert:"Open en dicht omdraaien",label:"Label",color:"Kleur",date_format:"Datumnotatie"};function mt(i=new Date){let e=i.getHours();return e<6?"Goedenacht":e<12?"Goedemorgen":e<18?"Goedemiddag":"Goedenavond"}var gt=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],ft=["januari","februari","maart","april","mei","juni","juli","augustus","september","oktober","november","december"],be={humidity:{icon:"drop",tone:"water",label:"Luchtvochtigheid"},wind:{icon:"wind",tone:"neutral",label:"Wind"},uv:{icon:"uv",tone:"solar",label:"UV-index"},precipitation:{icon:"rain",tone:"water",label:"Neerslag"},pressure:{icon:"gaugeArrow",tone:"neutral",label:"Luchtdruk"},sunrise:{icon:"sunrise",tone:"warn",label:"Zonsopkomst"},sunset:{icon:"sunset",tone:"warn",label:"Zonsondergang"}},bt=["humidity","wind","uv","precipitation","sunset"],vt=i=>i==null||Number.isNaN(+i)?"":["N","NO","O","ZO","Z","ZW","W","NW"][Math.round(+i/45)%8],ve=class extends _{static css=`
  ...
  `;validate(e){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768,...e}}watched(){let e=this.config;return[e.weather,e.weather_uv,e.sun,e.precipitation_entity].filter(Boolean)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),e.show_rule===!1&&this.setAttribute("no-rule",""),`
      ...
      </div>`}wire(){let e=()=>{let n=6e4-Date.now()%6e4+50;this.timer_=setTimeout(()=>{this.paintClock_(),e()},n)};e(),this.teardown_.push(()=>clearTimeout(this.timer_));let t=Number(this.config.hide_below)||0;if(t>0){let n=matchMedia(`(max-width: ${t-1}px)`),a=()=>this.toggleAttribute("narrow",n.matches);a(),n.addEventListener("change",a),this.teardown_.push(()=>n.removeEventListener("change",a))}}paintClock_(){let e=new Date,t=this.config.name??this.hass?.user?.name??"",n=mt(e);this.$(".hello").innerHTML=t?`${n}, <b>${t}</b>`:n,this.text(".date",`${gt[e.getDay()]} ${e.getDate()} ${ft[e.getMonth()]}`);let a=this.$(".clock");a&&this.text(a,e.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"}))}paint(){this.paintClock_();let e=this.config,t=f(this.hass,e.weather),n=M(this.hass,e.weather),a=this.$(".now");if(a&&t){let l=Ue(t.state);a.style.setProperty("--wtone",C(e.tone,"water"));let h=this.hass?.config?.unit_system?.temperature??"\xB0C";this.$(".temp").innerHTML=n.temperature!=null?`${N(this.hass,n.temperature,0)}<span>${h}</span>`:"--";let p=a.querySelector(".ic");p.dataset.icon!==l&&(p.dataset.icon=l,p.innerHTML=$(l,"cloud")),this.text(a.querySelector(".cond"),D(this.hass,t))}let o=this.$(".chips");if(!o)return;let r=bt.map(l=>this.chip_(l,n)).filter(Boolean),s=r.map(l=>`${l.key}${l.value}`).join("|");o.dataset.sig!==s&&(o.dataset.sig=s,o.innerHTML=r.map(l=>`<span class="chip2" style="--tone:${C(be[l.key].tone)}" title="${be[l.key].label}">
             ${x[be[l.key].icon]??""}${l.value}
           </span>`).join(""))}chip_(e,t){let n=this.config;switch(e){case"humidity":return t.humidity!=null?{key:e,value:`${Math.round(t.humidity)}%`}:null;case"wind":{if(t.wind_speed==null)return null;let a=this.hass?.config?.unit_system?.wind_speed??"km/h",o=vt(t.wind_bearing);return{key:e,value:`${N(this.hass,t.wind_speed,0)} ${a}${o?` ${o}`:""}`}}case"uv":{let o=M(this.hass,n.weather_uv).uv_index??t.uv_index??(n.weather_uv?Number(f(this.hass,n.weather_uv)?.state):null);return o!=null&&!Number.isNaN(+o)?{key:e,value:`UV ${N(this.hass,o,1)}`}:null}case"precipitation":{let a=f(this.hass,n.precipitation_entity);if(a){let o=Number(a.state);if(Number.isNaN(o))return null;let r=a.attributes.unit_of_measurement??"mm";return{key:e,value:`${N(this.hass,o,1)} ${r}`}}return t.precipitation!=null&&!Number.isNaN(+t.precipitation)?{key:e,value:`${N(this.hass,t.precipitation,1)} mm`}:null}case"pressure":return t.pressure!=null?{key:e,value:`${N(this.hass,t.pressure,0)} ${t.pressure_unit??"hPa"}`}:null;case"sunset":case"sunrise":{let o=f(this.hass,n.sun)?.attributes?.[e==="sunset"?"next_setting":"next_rising"];if(!o)return null;let r=new Date(o);return Number.isNaN(+r)?null:{key:e,value:r.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"})}}default:return null}}getCardSize(){return 2}getGridOptions(){return{columns:"full",rows:2,min_rows:2,max_rows:2}}static getConfigElement(){return document.createElement("domotiapp-header-card-editor")}static getStubConfig(e){return{weather:Object.keys(e?.states??{}).find(n=>n.startsWith("weather.")),sun:"sun.sun"}}},xe=class extends E{defaults(){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768}}pickers(){return[{key:"tone",kind:"tone",label:"Kleur weericoon"}]}schema(){return[ge({name:"weather",selector:g.entity("weather")},{name:"weather_uv",selector:{entity:{domain:["weather","sensor"]}}}),ge({name:"sun",selector:g.entity("sun")},{name:"precipitation_entity",selector:g.entity("sensor")}),{name:"name",selector:g.text()},{name:"hide_below",selector:g.number(0,1400,8)}]}label(e){return{weather:"Weer (temperatuur, wind)",weather_uv:"Tweede weerbron (UV-index)",precipitation_entity:"Neerslagsensor",show_rule:"Accentlijn tonen",hide_below:"Verbergen onder breedte (px)",bare:"Zonder kaartrand",name:"Naam"}[e.name]??super.label(e)}helper(e){if(e.name==="weather_uv")return"Alleen voor de UV-index. Handig als je hoofdbron die niet meelevert.";if(e.name==="precipitation_entity")return"Een sensor in mm of mm/h, bijvoorbeeld neerslagintensiteit of regen laatste uur.";if(e.name==="hide_below")return"768 verbergt de header op telefoons en houdt hem op tablets en desktops. 0 zet het uit.";if(e.name==="name")return"Leeg laten voor de naam van de ingelogde gebruiker."}};j("domotiapp-header-card-editor",xe);S("domotiapp-header-card",ve,{name:"DomotiApp Header",description:"Smalle strip met begroeting, weer en klok. Verbergt zichzelf op telefoons."});var we=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return{icon:"",tone:"accent",line:!0,...e}}watched(){return this.config.secondary_entity?[this.config.secondary_entity]:[]}template(){let e=this.config,t=e.icon!==null&&e.icon!==!1;return t||this.setAttribute("no-icon",""),`
      <div class="sep" style="--tone:${C(e.tone)}">
        ${t?`<span class="chip">${$(e.icon,"star")}</span>`:""}
        <h3></h3>
        ${e.line===!1?"":'<span class="rule"></span>'}
        <span class="sub"><span class="si"></span><span class="sv"></span></span>
      </div>`}paint(){this.text("h3",this.config.name??"");let e=this.$(".sub");if(!e)return;let t=f(this.hass,this.config.secondary_entity),n=e.querySelector(".si"),a=e.querySelector(".sv");if(!t){a.textContent="",n.innerHTML="";return}let o=this.config.secondary_icon??"";n.dataset.icon!==o&&(n.dataset.icon=o,n.innerHTML=o?$(o):"");let r=t.attributes.unit_of_measurement;a.textContent=r?`${t.state} ${r}`:t.attributes.current_temperature!=null?`${t.attributes.current_temperature} \xB0C`:D(this.hass,t)}getCardSize(){return 1}getGridOptions(){return{columns:"full",rows:1,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-separator-card-editor")}static getStubConfig(){return{name:"Nieuwe sectie",icon:"house",tone:"accent"}}},ye=class extends E{defaults(){return{line:!0,tone:"accent"}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon links",fallback:"star",auto:!1},{key:"tone",kind:"tone",label:fe.tone},{key:"secondary_icon",kind:"icon",label:"Icoon bij de waarde rechts",auto:!1}]}schema(){return[{name:"name",selector:g.text()},{name:"line",selector:g.bool()},{name:"secondary_entity",selector:g.entity()}]}label(e){return{line:"Lijn tonen",secondary_entity:"Waarde rechts (optioneel)"}[e.name]??super.label(e)}helper(e){if(e.name==="secondary_entity")return"Toont de status van deze entiteit rechts van de lijn, bijvoorbeeld een temperatuur of een aantal."}};j("domotiapp-separator-card-editor",ye);S("domotiapp-separator-card",we,{name:"DomotiApp Separator",description:"Sectiekop met icoon en vervagende lijn."});var _e=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return{layout:"row",show_state:!0,show_name:!0,show_icon:!0,...e}}watched(){return[this.config.entity].filter(Boolean)}tone_(){let e=this.config;return e.tone?C(e.tone):L(e.entity)!=="light"?T.accent:Q(f(this.hass,e.entity))??T.lit}template(){let e=this.config;return this.setAttribute("layout",["row","tile","compact"].includes(e.layout)?e.layout:"row"),`
      ...
      </div>`}wire(){let e=this.config,t=(a,o)=>ne(this,this.hass,e,e[a]??o);this.teardown_.push(A(this.$(".btn"),{onTap:()=>t("tap_action",{action:e.entity?"more-info":"none"}),onHold:()=>t("hold_action",{action:e.entity?"more-info":"none"}),onDouble:e.double_tap_action?()=>t("double_tap_action",{action:"none"}):void 0}));let n=this.$(".chip");n&&(this.teardown_.push(A(n,{onTap:a=>t("icon_tap_action",te(e.entity)),onHold:()=>t("icon_hold_action",{action:e.entity?"more-info":"none"})})),this.on(n,"click",a=>a.stopPropagation()),this.on(n,"pointerdown",a=>a.stopPropagation()))}paint(){let e=this.config,t=f(this.hass,e.entity),n=ee(t),a=!!e.entity&&I(t);this.toggleAttribute("on",n),this.$(".btn").classList.toggle("unavailable",a),this.$(".btn").style.setProperty("--tone",this.tone_());let o=this.$(".chip");if(o){let s=J(this.hass,e.entity,e.icon),l=s?`pic:${s}`:e.icon||U(e.entity,M(this.hass,e.entity));o.dataset.icon!==l&&(o.dataset.icon=l,o.classList.toggle("pic",!!s),o.innerHTML=s?`<img src="${s}" alt="" loading="lazy" />`:$(l)),o.style.setProperty("--tone",n&&!s?this.tone_():"var(--dac-ink-3)"),o.setAttribute("aria-label",e.entity?`${z(this.hass,e.entity,e.name)} schakelen`:"Icoon")}this.text(".nm",z(this.hass,e.entity,e.name));let r=this.$(".st");r&&this.text(r,this.secondary_(t,a)),this.$(".btn").setAttribute("aria-label",`${z(this.hass,e.entity,e.name)}${t?`, ${D(this.hass,t)}`:""}`)}secondary_(e,t){return t?"Niet bereikbaar":!e||B(e.entity_id)?"":L(e.entity_id)==="light"&&e.state==="on"&&e.attributes.brightness!=null?`${Math.round(e.attributes.brightness/255*100)}%`:D(this.hass,e)}getCardSize(){return this.config?.layout==="tile"?2:1}getGridOptions(){return this.config?.layout==="tile"?{columns:6,rows:2,min_columns:3,min_rows:2,max_rows:2}:{columns:12,rows:1,min_columns:4,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-button-card-editor")}static getStubConfig(e,t){return{entity:t?.find(a=>a.startsWith("light."))??t?.find(a=>a.startsWith("switch."))??t?.[0],layout:"row"}}},ke=class extends E{defaults(){return{layout:"row",show_state:!0,show_name:!0,show_icon:!0,icon_tap_action:{action:"toggle"},tap_action:{action:"more-info"}}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"star"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"entity",selector:g.entity()},{name:"name",selector:g.text()},{name:"layout",selector:g.select([{value:"row",label:"Rij"},{value:"tile",label:"Tegel"},{value:"compact",label:"Compact"}])},{name:"show_icon",selector:g.bool()},{name:"show_name",selector:g.bool()},{name:"show_state",selector:g.bool()},{name:"icon_tap_action",selector:g.action("toggle")},{name:"icon_hold_action",selector:g.action("more-info")},{name:"tap_action",selector:g.action("more-info")},{name:"hold_action",selector:g.action("more-info")},{name:"double_tap_action",selector:g.action("none")}]}label(e){return{entity:"Entiteit",name:"Naam (overschrijft die van de entiteit)",layout:"Vorm",show_icon:"Icoon tonen",show_name:"Naam tonen",show_state:"Status tonen",icon_tap_action:"Tikken op het icoon",icon_hold_action:"Vasthouden op het icoon",tap_action:"Tikken op de kaart",hold_action:"Vasthouden op de kaart",double_tap_action:"Dubbeltikken op de kaart"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"Mag leeg blijven: zonder entiteit wordt dit een navigatieknop.";if(e.name==="icon_tap_action")return"Handig: het icoon schakelt de lichtgroep, de kaart navigeert naar de ruimte.";if(e.name==="tap_action")return"Wat er gebeurt als je naast het icoon tikt, bijvoorbeeld navigeren naar een pop-up."}};j("domotiapp-button-card-editor",ke);S("domotiapp-button-card",_e,{name:"DomotiApp Knop",description:"E\xE9n control als rij, tegel of compacte pil. Vervangt tile, mushroom-entity en bubble-knoppen."});var Me=(i,e,t)=>Math.min(t,Math.max(e,i));function ae(i,e){let t=e.min??0,n=e.max??100,a=e.step??1,o=!1,r=d=>{let v=i.getBoundingClientRect();if(!v.width)return t;let b=Me((d-v.left)/v.width,0,1),y=t+b*(n-t);return Me(Math.round(y/a)*a,t,n)},s=d=>{try{i.setPointerCapture?.(d)}catch{}},l=d=>{try{i.hasPointerCapture?.(d)&&i.releasePointerCapture(d)}catch{}},h=d=>{e.disabled?.()||d.button!=null&&d.button!==0||(o=!0,s(d.pointerId),i.classList.add("dragging"),e.onInput(r(d.clientX)),d.preventDefault())},p=d=>{o&&(e.onInput(r(d.clientX)),d.preventDefault())},u=d=>{o&&(o=!1,l(d.pointerId),i.classList.remove("dragging"),e.onCommit(r(d.clientX)))},m=d=>{o&&(o=!1,l(d?.pointerId),i.classList.remove("dragging"),e.onInput(e.value()))},k=d=>{if(e.disabled?.())return;let v=(n-t)/10,b={ArrowLeft:-a,ArrowDown:-a,ArrowRight:a,ArrowUp:a,PageDown:-v,PageUp:v,Home:-1/0,End:1/0};if(!(d.key in b))return;d.preventDefault();let y=e.value(),q=Me(b[d.key]===-1/0?t:b[d.key]===1/0?n:y+b[d.key],t,n);e.onInput(q),e.onCommit(q)};return i.addEventListener("pointerdown",h),i.addEventListener("pointermove",p),i.addEventListener("pointerup",u),i.addEventListener("pointercancel",m),i.addEventListener("keydown",k),()=>{i.removeEventListener("pointerdown",h),i.removeEventListener("pointermove",p),i.removeEventListener("pointerup",u),i.removeEventListener("pointercancel",m),i.removeEventListener("keydown",k)}}var W=(i="")=>`
...
`;var xt=new Set(["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"]),wt=new Set(["hs","rgb","rgbw","rgbww","xy"]),Ee=i=>i?.attributes?.supported_color_modes??[],yt=i=>Ee(i).some(e=>xt.has(e)),re=i=>Ee(i).some(e=>wt.has(e)),se=i=>Ee(i).includes("color_temp"),Fe=i=>Math.max(1,Math.round((i??0)/255*100)),$e=class extends _{static css=`
    :host { display: block; }
  ...
  `;validate(e){let t=e.entity??e.lights?.[0]??e.entities?.[0],n=typeof t=="string"?t:t?.entity;return n?{show_colour:!0,...e,entity:n}:{...e,[O]:"Kies een lamp."}}watched(){return[this.config.entity]}template(){return this.config.bare&&this.setAttribute("bare",""),`
      ...
      </div>`}wire(){let e=this.config.entity;this.teardown_.push(A(this.$(".chip"),{onTap:()=>this.hass.callService("light","toggle",{entity_id:e}),onHold:()=>P(this,e)})),this.on(this.$(".card"),"click",t=>{t.target.closest(".toggle")&&this.hass.callService("light","toggle",{entity_id:e})}),this.sliders_=new Map}attach_(e,t,n){if(!e||this.sliders_.has(t))return;let a=ae(e,n);this.sliders_.set(t,a),this.teardown_.push(a)}setSlider_(e,t,n=0,a=100){if(!e)return;let o=a>n?(t-n)/(a-n)*100:0;e.style.setProperty("--v",`${o}%`),e.setAttribute("aria-valuemin",String(n)),e.setAttribute("aria-valuemax",String(a)),e.setAttribute("aria-valuenow",String(t))}paint(){let e=this.config,t=f(this.hass,e.entity),n=I(t),a=t?.state==="on",o=this.$(".lamp");o.dataset.on=String(a),o.classList.toggle("unavailable",n);let r=this.$(".chip"),s=e.icon||"bulb";r.dataset.icon!==s&&(r.dataset.icon=s,r.innerHTML=$(s,"bulb")),this.text(".nm",z(this.hass,e.entity,e.name));let l=a?t?.attributes?.rgb_color:null;o.style.setProperty("--tone",l?`rgb(${l[0]},${l[1]},${l[2]})`:"var(--dac-lit)");let h=this.$(".ctl"),p=n?"none":yt(t)?"range":"toggle";if(h.dataset.kind!==p&&(h.dataset.kind=p,h.innerHTML=p==="range"?W("brightness"):p==="toggle"?'<button class="toggle" type="button" role="switch" aria-checked="false" aria-label="Aan of uit"></button>':"",this.sliders_.delete("brightness")),p==="range"){let u=h.querySelector(".slider");if(this.attach_(u,"brightness",{value:()=>t?.state==="on"?Fe(f(this.hass,e.entity)?.attributes?.brightness):0,onInput:m=>{this.setSlider_(u,m),this.text(".v",m===0?"Uit":`${m}%`)},onCommit:m=>{m===0?this.hass.callService("light","turn_off",{entity_id:e.entity}):this.hass.callService("light","turn_on",{entity_id:e.entity,brightness_pct:m})},disabled:()=>I(f(this.hass,e.entity))}),!u.classList.contains("dragging")){let m=a?Fe(t.attributes.brightness):0;this.setSlider_(u,m),this.text(".v",a?`${m}%`:"Uit")}}else p==="toggle"?(h.querySelector(".toggle")?.setAttribute("aria-checked",String(a)),this.text(".v",a?"Aan":"Uit")):this.text(".v","Niet bereikbaar");this.paintColour_(t,a)}paintColour_(e,t){let n=this.$(".colour"),a=this.config.show_colour!==!1&&(re(e)||se(e));if(n.hidden=!(a&&t),!a)return;let o=`${re(e)?"c":""}${se(e)?"t":""}`;if(n.dataset.sig!==o){n.dataset.sig=o,n.innerHTML=(re(e)?`<span data-kind="hue" style="display:contents">${W("hue")}</span>`:"")+(se(e)?`<span data-kind="kelvin" style="display:contents">${W("kelvin")}</span>`:"");let h=n.querySelector(".slider.hue");h&&(h.dataset.strip="",h.style.setProperty("--strip","linear-gradient(90deg, hsl(0 90% 55%), hsl(60 90% 55%), hsl(120 90% 55%), hsl(180 90% 55%), hsl(240 90% 55%), hsl(300 90% 55%), hsl(360 90% 55%))"),h.setAttribute("aria-label","Kleur"));let p=n.querySelector(".slider.kelvin");p&&(p.dataset.strip="",p.style.setProperty("--strip","linear-gradient(90deg,#ffb15e,#ffd6a8,#fff5e8,#eaf1ff,#cbdcff)"),p.setAttribute("aria-label","Kleurtemperatuur")),this.sliders_.delete("hue"),this.sliders_.delete("kelvin")}if(!t)return;let r=this.config.entity,s=n.querySelector(".slider.hue");s&&(this.attach_(s,"hue",{min:0,max:360,value:()=>f(this.hass,r)?.attributes?.hs_color?.[0]??0,onInput:h=>this.setSlider_(s,h,0,360),onCommit:h=>{let p=f(this.hass,r)?.attributes?.hs_color?.[1]??100;this.hass.callService("light","turn_on",{entity_id:r,hs_color:[h,p]})}}),s.classList.contains("dragging")||this.setSlider_(s,Math.round(e.attributes.hs_color?.[0]??0),0,360));let l=n.querySelector(".slider.kelvin");if(l){let h=e.attributes.min_color_temp_kelvin??2e3,p=e.attributes.max_color_temp_kelvin??6500;if(this.attach_(l,"kelvin",{min:h,max:p,step:50,value:()=>f(this.hass,r)?.attributes?.color_temp_kelvin??h,onInput:u=>this.setSlider_(l,u,h,p),onCommit:u=>this.hass.callService("light","turn_on",{entity_id:r,color_temp_kelvin:u})}),!l.classList.contains("dragging")){let u=e.attributes.color_temp_kelvin;u!=null&&this.setSlider_(l,u,h,p)}}}getCardSize(){let e=f(this.hass,this.config?.entity);return e?.state==="on"&&(re(e)||se(e))?2:1}getGridOptions(){return{columns:12,rows:"auto",min_columns:4,min_rows:1}}static getConfigElement(){return document.createElement("domotiapp-light-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("light."));return n?{entity:n}:{}}},Se=class extends E{defaults(){return{show_colour:!0}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"bulb"}]}schema(){return[{name:"entity",selector:g.entity("light")},{name:"name",selector:g.text()},{name:"show_colour",selector:g.bool()}]}label(e){return{entity:"Lamp",name:"Naam (overschrijft die van de lamp)",show_colour:"Kleurstrips tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"E\xE9n lamp per kaart. Dimbaar krijgt een schuif, alleen schakelbaar een tuimelaar.";if(e.name==="show_colour")return"Kleur en kleurtemperatuur verschijnen zodra de lamp aan is. De kaart is dan twee rijen hoog."}};j("domotiapp-light-card-editor",Se);S("domotiapp-light-card",$e,{name:"DomotiApp Verlichting",description:"E\xE9n lamp op \xE9\xE9n rasterrij: dimmen, kleur en kleurtemperatuur."});function Ze(i){if(!i)return null;let e=Number(i.state);return Number.isFinite(e)?e:null}function _t(i){let e=i?.attributes?.hvac_action;return e||(i?.state==="off"?"off":i?.state==="cool"?"cooling":i?.state==="heat"?"idle":null)}var ze={heating:"var(--dac-solar)",cooling:"var(--dac-grid-in)",drying:"var(--dac-grid-in)",fan:"var(--dac-grid-in)"},Xe={heating:"Verwarmt",cooling:"Koelt",drying:"Ontvochtigt",fan:"Ventileert",idle:"Uit",off:"Uit"},Ce=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return e.entity||e.temperature||e.humidity?{...e}:{...e,[O]:"Kies een thermostaat, of een temperatuursensor."}}watched(){let e=this.config;return[e.entity,e.temperature,e.humidity].filter(Boolean)}step_(){let e=M(this.hass,this.config.entity);return Number(this.config.step??e.target_temp_step)||.5}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),e.entity||this.setAttribute("readout",""),`
      <div class="card surface">
        ...
        ${e.entity?`<div class="set">
                 <button type="button" data-d="-1" aria-label="Lager">${x.minus}</button>
                 <span class="target tnum"></span>
                 <button type="button" data-d="1" aria-label="Hoger">${x.plus}</button>
               </div>`:""}
      </div>`}wire(){let e=this.config;this.teardown_.push(()=>clearTimeout(this.sendTimer_)),this.teardown_.push(A(this.$(".chip"),{onTap:()=>P(this,e.entity||e.temperature||e.humidity)}));let t=this.$(".set");t&&t.querySelectorAll("button").forEach(n=>this.on(n,"click",()=>this.nudge_(Number(n.dataset.d))))}nudge_(e){let t=this.config,n=M(this.hass,t.entity),a=this.step_(),o=Number(n.min_temp??5),r=Number(n.max_temp??35),s=this.pending_??Number(n.temperature);if(!Number.isFinite(s))return;let l=Math.min(r,Math.max(o,Math.round((s+e*a)/a)*a));this.pending_=l,this.paintTarget_(),clearTimeout(this.sendTimer_),this.sendTimer_=setTimeout(()=>{this.sendTimer_=null,this.hass.callService("climate","set_temperature",{entity_id:t.entity,temperature:this.pending_}),setTimeout(()=>{this.pending_=null,this.paint()},1500)},450)}paintTarget_(){let e=this.$(".target");if(!e)return;let t=M(this.hass,this.config.entity),n=this.pending_??Number(t.temperature);e.classList.toggle("pending",this.pending_!=null),e.textContent=Number.isFinite(n)?`${N(this.hass,n,n%1?1:0)}\xB0`:"--"}paint(){let e=this.config,t=e.entity?f(this.hass,e.entity):null,n=e.entity?I(t):!1;this.toggleAttribute("dead",n);let a=_t(t),o=e.tone?C(e.tone):ze[a]??"var(--dac-ink-3)";this.$(".card").style.setProperty("--tone",o),this.toggleAttribute("busy",!!ze[a]);let r=this.$(".chip"),s=e.icon||"thermo";r.dataset.icon!==s&&(r.dataset.icon=s,r.innerHTML=$(s,"thermo")),r.style.setProperty("--tone",ze[a]?o:"var(--dac-ink-3)"),this.text(".nm",z(this.hass,e.entity||e.temperature||e.humidity,e.name));let l=e.temperature?Ze(f(this.hass,e.temperature)):Number(M(this.hass,e.entity).current_temperature),h=this.hass?.config?.unit_system?.temperature??"\xB0C";this.text(".temp",Number.isFinite(l)?`${N(this.hass,l,1)} ${h}`:"--");let p=e.humidity?Ze(f(this.hass,e.humidity)):null,u=this.$(".hum");u.innerHTML=p==null?"":`${x.drop}${N(this.hass,p,0)}%`,this.text(".sep",p==null?"":"\xB7"),e.entity&&!e.humidity&&Xe[a]&&a!=="idle"&&(this.text(".sep","\xB7"),u.textContent=Xe[a]),this.paintTarget_();let m=this.$(".set");if(m){let k=M(this.hass,e.entity),d=this.pending_??Number(k.temperature);m.querySelector('[data-d="-1"]').disabled=n||d<=Number(k.min_temp??5),m.querySelector('[data-d="1"]').disabled=n||d>=Number(k.max_temp??35)}}getCardSize(){return 1}getGridOptions(){return{columns:12,rows:1,min_columns:4,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-climate-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("climate."));return n?{entity:n}:{}}},je=class extends E{pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"thermo"},{key:"tone",kind:"tone",label:"Vaste kleur (leeg = volgt de ketel)"}]}schema(){return[{name:"entity",selector:g.entity("climate")},{name:"temperature",selector:{entity:{domain:"sensor",device_class:"temperature"}}},{name:"humidity",selector:{entity:{domain:"sensor",device_class:"humidity"}}},{name:"name",selector:g.text()},{name:"step",selector:g.number(.1,5,.1)}]}label(e){return{entity:"Thermostaat (optioneel)",temperature:"Temperatuursensor (optioneel)",humidity:"Vochtigheidssensor (optioneel)",name:"Naam",step:"Stap van de knoppen"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"Leeg laten voor een kaart die alleen meet. Met thermostaat komen de stelknoppen erbij.";if(e.name==="temperature")return"Wint van de meting van de thermostaat zelf. Handig als er een betere sensor in de kamer hangt.";if(e.name==="step")return"Leeg laten volgt de thermostaat, en anders een halve graad."}};j("domotiapp-climate-card-editor",je);S("domotiapp-climate-card",Ce,{name:"DomotiApp Klimaat",description:"Thermostaat, losse temperatuur- en vochtsensor, of allebei."});var Ye=i=>Math.min(Math.max(1,Number(i)||2),3),Je=i=>typeof i=="string"?{entity:i}:{...i};function ce(i){for(i.bewaard??=[];i.items.length<i.columns;)i.items.push(i.bewaard.pop()??{entity:""});for(;i.items.length>i.columns;){let e=i.items.pop();e.entity&&i.bewaard.push(e)}return i}function kt(i){let e=Array.isArray(i.rows)&&i.rows.length?i.rows.map(n=>({columns:Ye(n.columns),items:(n.items??n.entities??[]).map(Je)})):(()=>{let n=(i.items??i.entities??[]).map(Je);return n.length?[{columns:Ye(i.columns),items:n}]:[]})(),t=[];for(let n of e){let a=[];for(let o=0;o<n.items.length;o+=n.columns)a.push(n.items.slice(o,o+n.columns));a.length||a.push([]);for(let o of a)t.push(ce({columns:n.columns,items:o}))}return t}var Qe=i=>i.map(e=>({columns:e.columns,items:e.items.filter(t=>t.entity).map(t=>structuredClone(t))})).filter(e=>e.items.length),Mt=`
...
`,Oe=class extends HTMLElement{constructor(){super(),this.rows_=[],this.rest_={},this.open_=new Set,this.koppen_=[]}setConfig(e){if(this.rest_={...e},delete this.rest_.rows,delete this.rest_.items,delete this.rest_.entities,delete this.rest_.columns,this.gebouwd_&&e===this.uitObject_)return;let t=kt(e);this.gebouwd_&&JSON.stringify(Qe(t))===this.uit_||(this.rows_=t,this.eersteKeer_||(this.eersteKeer_=!0,this.rows_.length===1&&this.open_.add("r0")),this.build_())}set hass(e){this.hass_=e;for(let t of this.querySelectorAll("ha-form, dac-icon-picker, dac-tone-picker"))t.hass=e;this.gebouwd_||this.build_()}get hass(){return this.hass_}connectedCallback(){this.gebouwd_||this.build_()}onthoud_(e,t){e.open=this.open_.has(t),e.addEventListener("toggle",()=>{e.open?this.open_.add(t):this.open_.delete(t)})}rijWeg_(e){let t=new Set;for(let n of this.open_){let a=/^r(\d+)(?:i(\d+))?$/.exec(n);if(!a)continue;let o=Number(a[1]);o!==e&&t.add(o>e?`r${o-1}${a[2]===void 0?"":`i${a[2]}`}`:n)}this.open_=t}itemWeg_(e,t){let n=new Set;for(let a of this.open_){let o=/^r(\d+)i(\d+)$/.exec(a);if(!o||Number(o[1])!==e){n.add(a);continue}let r=Number(o[2]);r!==t&&n.add(r>t?`r${e}i${r-1}`:a)}this.open_=n}legePlekkenOpen_(e,t){e.items.forEach((n,a)=>{n.entity||this.open_.add(`r${t}i${a}`)})}async build_(){if(!this.hass_||!this.rows_)return;await customElements.whenDefined("ha-form"),this.gebouwd_=!0,this.replaceChildren(),this.koppen_=[];let e=document.createElement("style");e.textContent=Mt;let t=document.createElement("div");if(t.className="dac-ed",this.append(e,t),this.rows_.forEach((a,o)=>t.appendChild(this.rijBlok_(a,o))),!this.rows_.length){let a=document.createElement("p");a.className="uitleg",a.textContent="Een rij is \xE9\xE9n regel op de kaart, met \xE9\xE9n, twee of drie entiteiten naast elkaar. Elke rij heeft zijn eigen indeling.",t.appendChild(a)}let n=document.createElement("button");n.type="button",n.className="rijtoevoegen",n.textContent="\uFF0B  Rij toevoegen",n.addEventListener("click",()=>{let a=ce({columns:2,items:[]});this.rows_.push(a);let o=this.rows_.length-1;this.open_.add(`r${o}`),this.legePlekkenOpen_(a,o),this.emit_(),this.build_()}),t.appendChild(n)}binnenKop_(e,t){return e.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),t(n)}),e}rijBlok_(e,t){let n=document.createElement("details");n.className="rij",this.onthoud_(n,`r${t}`);let a=document.createElement("summary"),o=document.createElement("span");o.className="pijl",o.textContent="\u203A";let r=document.createElement("span");r.className="titel";let s=document.createElement("b");s.textContent=`Rij ${t+1}`;let l=document.createElement("small");r.append(s,l);let h=document.createElement("span");h.className="kolommen";let p=[1,2,3].map(d=>{let v=document.createElement("button");return v.type="button",v.textContent=String(d),v.title=`${d} entiteit${d>1?"en":""} in deze rij`,h.appendChild(this.binnenKop_(v,()=>{e.columns!==d&&(e.columns=d,ce(e),this.open_.add(`r${t}`),this.legePlekkenOpen_(e,t),this.emit_(),this.build_())})),v}),u=()=>{let d=e.items.filter(b=>b.entity),v=`${e.columns} kolom${e.columns>1?"men":""}`;l.textContent=d.length?`${v} \xB7 ${d.map(b=>this.itemNaam_(b)).join(", ")}`:`${v} \xB7 nog leeg`,p.forEach(b=>b.setAttribute("aria-pressed",String(e.columns===Number(b.textContent))))};this.koppen_.push(u);let m=document.createElement("button");m.type="button",m.className="weg",m.title="Rij verwijderen",m.textContent="\u2715",this.binnenKop_(m,()=>{this.rows_.splice(t,1),this.rijWeg_(t),this.emit_(),this.build_()}),a.append(o,r,h,m);let k=document.createElement("div");return k.className="rijbody",e.items.forEach((d,v)=>k.appendChild(this.itemBlok_(e,d,t,v))),n.append(a,k),u(),n}itemNaam_(e){return e.name||this.hass_?.states?.[e.entity]?.attributes?.friendly_name||e.entity}itemBlok_(e,t,n,a){let o=document.createElement("details");o.className="item",this.onthoud_(o,`r${n}i${a}`);let r=document.createElement("summary"),s=document.createElement("span");s.className="pijl",s.textContent="\u203A";let l=document.createElement("span");l.className="nr",l.textContent=String(a+1),l.title=`Plek ${a+1} in de rij`;let h=document.createElement("span");h.className="titel";let p=document.createElement("b"),u=document.createElement("small");h.append(p,u);let m=document.createElement("button");m.type="button",m.className="weg",m.title="Deze plek leegmaken",m.textContent="\u2715",this.binnenKop_(m,()=>{e.items.splice(a,1),this.itemWeg_(n,a),ce(e),this.emit_(),this.build_()}),r.append(s,l,h,m);let k=document.createElement("div");k.className="itembody";let d=document.createElement("ha-form");d.hass=this.hass_,d.schema=[{name:"entity",selector:{entity:{}}}],d.computeLabel=()=>"Entiteit",d.addEventListener("value-changed",w=>{w.stopPropagation(),t.entity=w.detail.value.entity??"",this.emit_()});let v=document.createElement("dac-icon-picker");v.label="Icoon",v.hass=this.hass_,v.addEventListener("value-changed",w=>{w.stopPropagation(),w.detail.value?t.icon=w.detail.value:delete t.icon,this.emit_()});let b=document.createElement("dac-tone-picker");b.label="Kleur",b.hass=this.hass_,b.addEventListener("value-changed",w=>{w.stopPropagation(),w.detail.value?t.tone=w.detail.value:delete t.tone,this.emit_()});let y=document.createElement("ha-form");y.hass=this.hass_,y.schema=[{name:"name",selector:{text:{}}},{name:"show_state",selector:{boolean:{}}},{name:"icon_tap_action",selector:{ui_action:{default_action:"toggle"}}},{name:"icon_hold_action",selector:{ui_action:{default_action:"more-info"}}},{name:"tap_action",selector:{ui_action:{default_action:"more-info"}}},{name:"hold_action",selector:{ui_action:{default_action:"more-info"}}}],y.computeLabel=w=>({name:"Naam (overschrijft die van de entiteit)",show_state:"Status tonen",icon_tap_action:"Tikken op het icoon",icon_hold_action:"Vasthouden op het icoon",tap_action:"Tikken op de regel",hold_action:"Vasthouden op de regel"})[w.name]??w.name,y.computeHelper=w=>w.name==="icon_tap_action"?"Het icoon en de regel zijn twee knoppen: het icoon schakelt, de regel opent of navigeert.":void 0,y.addEventListener("value-changed",w=>{w.stopPropagation();let K=w.detail.value;K.name?t.name=K.name:delete t.name,K.show_state===!1?t.show_state=!1:delete t.show_state;for(let X of["icon_tap_action","icon_hold_action","tap_action","hold_action"])K[X]?t[X]=K[X]:delete t[X];this.emit_()});let q=()=>{p.textContent=t.entity?this.itemNaam_(t):"Kies een entiteit",u.textContent=t.entity||"",o.dataset.leeg=String(!t.entity),m.hidden=!t.entity};return this.koppen_.push(q),d.data={entity:t.entity||void 0},v.value=t.icon??"",b.value=t.tone??"",y.data={name:t.name??"",show_state:t.show_state??!0,icon_tap_action:t.icon_tap_action,icon_hold_action:t.icon_hold_action,tap_action:t.tap_action,hold_action:t.hold_action},k.append(d,v,b,y),o.append(r,k),q(),o}emit_(){let e=Qe(this.rows_),t={...this.rest_,rows:e};this.uit_=JSON.stringify(e),this.uitObject_=t;for(let n of this.koppen_)n();this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}))}};customElements.get("domotiapp-entities-card-editor")||customElements.define("domotiapp-entities-card-editor",Oe);var et=44,Ae=6,tt=i=>typeof i=="string"?{entity:i}:{...i};function $t(i){if(Array.isArray(i.rows)&&i.rows.length)return i.rows.map(t=>({columns:Math.min(Math.max(1,Number(t.columns)||2),3),items:(t.items??t.entities??[]).map(tt)}));let e=(i.items??i.entities??[]).map(tt);return e.length?[{columns:Math.min(Math.max(1,Number(i.columns)||2),3),items:e}]:[]}var Le=class extends _{static css=`
    :host { display: block; height: 100%; }
    ...
    .it {
      display: flex; align-items: center; gap: 10px;
      min-height: ${et}px; padding: 2px 6px 2px 2px;
  ...
  `;validate(e){let t=$t(e);return t.some(n=>n.items.length)?{show_state:!0,...e,rows:t}:{...e,[O]:"Voeg een rij toe en kies daar entiteiten in."}}watched(){return this.config.rows.flatMap(e=>e.items.map(t=>t.entity))}item_(e,t){return this.config.rows[+e]?.items[+t]}tone_(e){return e.tone?C(e.tone):this.config.tone?C(this.config.tone):L(e.entity)!=="light"?T.accent:Q(f(this.hass,e.entity))??T.lit}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),this.style.containerType="inline-size",`<div class="card surface">${e.rows.map((n,a)=>`
      ...
      </div>`).join("")}</div>`}wire(){this.$$(".it").forEach(e=>{let t=this.item_(e.dataset.r,e.dataset.i);if(!t)return;let n=(o,r)=>ne(this,this.hass,t,t[o]??r);this.teardown_.push(A(e,{onTap:()=>n("tap_action",{action:"more-info"}),onHold:()=>n("hold_action",{action:"more-info"})}));let a=e.querySelector(".chip");this.teardown_.push(A(a,{onTap:()=>n("icon_tap_action",te(t.entity)),onHold:()=>n("icon_hold_action",{action:"more-info"})})),this.on(a,"click",o=>o.stopPropagation()),this.on(a,"pointerdown",o=>o.stopPropagation())})}paint(){this.$$(".it").forEach(e=>{let t=this.item_(e.dataset.r,e.dataset.i);if(!t)return;let n=f(this.hass,t.entity),a=ee(n),o=I(n);e.dataset.on=String(a),e.classList.toggle("unavailable",o);let r=this.tone_(t);e.style.setProperty("--tone",r);let s=e.querySelector(".chip"),l=J(this.hass,t.entity,t.icon),h=t.icon||(l?`pic:${l}`:U(t.entity,M(this.hass,t.entity)));s.dataset.icon!==h&&(s.dataset.icon=h,s.classList.toggle("pic",!!l),s.innerHTML=l?`<img src="${l}" alt="" loading="lazy" />`:$(t.icon||U(t.entity,M(this.hass,t.entity)))),s.style.setProperty("--tone",l?"var(--dac-ink-3)":a?r:"var(--dac-ink-3)");let p=z(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),p),s.setAttribute("aria-label",`${p} schakelen`);let u=e.querySelector(".st");if((t.show_state??this.config.show_state)===!1)u.textContent="";else if(o)u.textContent="Niet bereikbaar";else if(!n||B(n.entity_id))u.textContent="";else if(L(n.entity_id)==="light"&&a&&n.attributes.brightness!=null)u.textContent=`${Math.round(n.attributes.brightness/255*100)}%`;else{let k=n.attributes.unit_of_measurement;u.textContent=k?`${n.state} ${k}`:D(this.hass,n)}e.setAttribute("aria-label",`${p}${n?`, ${D(this.hass,n)}`:""}`)})}lines_(){return(this.config?.rows??[]).reduce((e,t)=>e+Math.ceil((t.items.length||1)/t.columns),0)}getCardSize(){return Math.max(1,this.lines_())}getGridOptions(){let e=Math.max(1,this.lines_()),t=H(12+e*et+(e-1)*Ae);return{columns:12,rows:t,min_columns:4,min_rows:t,max_rows:t}}static getConfigElement(){return document.createElement("domotiapp-entities-card-editor")}static getStubConfig(){return{rows:[]}}};S("domotiapp-entities-card",Le,{name:"DomotiApp Entiteiten",description:"Rijen entiteiten, elk met een eigen kolomindeling."});var F={OPEN:1,CLOSE:2,SET_POSITION:4,STOP:8},Z=(i,e)=>!!((i?.attributes?.supported_features??0)&e),St=(i={})=>{switch(i.device_class){case"garage":return{open:"garageOpen",closed:"garageClosed"};case"awning":case"blind":return{open:"awning",closed:"awning"};default:return{open:"shutterOpen",closed:"shutter"}}},Ne=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.covers??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,covers:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[O]:"Kies minstens \xE9\xE9n rolluik of zonnescherm."}}watched(){return this.config.covers.map(e=>e.entity)}keysHtml(e){return`
      <div class="keys">
        <button type="button" data-act="open" aria-label="Open">${x.arrowUp}</button>
        ${e?`<button type="button" data-act="stop" aria-label="Stop">${x.stop}</button>`:""}
        <button type="button" data-act="close" aria-label="Dicht">${x.arrowDown}</button>
      </div>`}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),`<div class="card surface">${e.covers.map((n,a)=>`
      <div class="cv" data-i="${a}" data-shown="closed" style="--tone:${C(n.tone??e.tone,"solar")}">
        <button class="chip" type="button" aria-label="Meer info"></button>
        <div class="txt"><div class="nm"></div><div class="st"></div></div>
        ${this.keysHtml(e.show_stop!==!1)}
        <div class="pos" hidden></div>
      </div>`).join("")}</div>`}wire(){this.dragging_=new Set,this.bound_=new Set,this.assumed_=new Map,this.$$(".cv").forEach(e=>{let t=e.dataset.i;e.querySelectorAll(".keys button").forEach(a=>{this.on(a,"click",()=>{let o=a.dataset.act,r={open:"open_cover",stop:"stop_cover",close:"close_cover"};this.hass.callService("cover",r[o],{entity_id:this.config.covers[+t].entity}),o!=="stop"&&(this.assumed_.set(t,o==="open"?"open":"closed"),this.paint())})});let n=this.config.covers[+t].entity;this.teardown_.push(A(e.querySelector(".chip"),{onTap:()=>P(this,n)}))})}paint(){this.$$(".cv").forEach(e=>{let t=e.dataset.i,n=this.config.covers[+t],a=f(this.hass,n.entity),o=M(this.hass,n.entity),r=!a||a.state==="unavailable",s=a?.state??"unknown";e.classList.toggle("unavailable",r),e.querySelector(".nm").textContent=z(this.hass,n.entity,n.name);let l=Z(a,F.SET_POSITION)&&o.current_position!=null,h=l?o.current_position>0?"open":"closed":s==="open"||s==="closed"?s:this.assumed_.get(t)??"closed";e.dataset.shown=h;let p=St(o),u=(h==="open"?n.icon_open:n.icon_closed)??(h==="open"?this.config.icon_open:this.config.icon_closed)??n.icon??p[h],m=e.querySelector(".chip");m.dataset.icon!==u&&(m.dataset.icon=u,m.innerHTML=$(u,p[h]));let k=e.querySelector(".st");this.dragging_.has(t)||(k.textContent=r?"Niet bereikbaar":s==="opening"?"Gaat open":s==="closing"?"Gaat dicht":l?`${o.current_position}% open`:s==="open"?"Open":s==="closed"?"Dicht":""),e.querySelectorAll(".keys button").forEach(b=>{if(b.dataset.act==="stop"){b.disabled=r||!Z(a,F.STOP);return}let y=b.dataset.act==="open";b.disabled=r||(y?!Z(a,F.OPEN):!Z(a,F.CLOSE))});let d=e.querySelector(".pos"),v=l&&this.config.show_position!==!1;if(d.hidden=!v,v){if(d.dataset.built||(d.dataset.built="1",d.innerHTML=W("position"),d.querySelector(".slider").setAttribute("aria-label","Positie")),!this.bound_.has(t)){this.bound_.add(t);let y=d.querySelector(".slider"),q=w=>{y.style.setProperty("--v",`${w}%`),y.setAttribute("aria-valuenow",String(w)),e.querySelector(".st").textContent=`${w}% open`};this.teardown_.push(ae(y,{value:()=>M(this.hass,n.entity).current_position??0,onInput:q,onCommit:w=>this.hass.callService("cover","set_cover_position",{entity_id:n.entity,position:w})}))}let b=d.querySelector(".slider");if(!b.classList.contains("dragging")){let y=o.current_position??0;b.style.setProperty("--v",`${y}%`),b.setAttribute("aria-valuenow",String(y))}}})}rows_(){let e=this.config?.covers??[],t=e.some(n=>Z(f(this.hass,n.entity),F.SET_POSITION));return H(12+Math.max(1,e.length)*42+(t?30:0))}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-cover-card-editor")}static getStubConfig(e,t){let n=t?.find(a=>a.startsWith("cover."));return{covers:n?[n]:[]}}},Te=class extends E{defaults(){return{show_stop:!0,show_position:!0}}pickers(){return[{key:"icon_open",kind:"icon",label:"Icoon als het open staat",fallback:"shutterOpen"},{key:"icon_closed",kind:"icon",label:"Icoon als het dicht is",fallback:"shutter"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"covers",selector:{entity:{domain:"cover",multiple:!0}}},{name:"show_stop",selector:g.bool()}]}label(e){return{covers:"Rolluiken",show_stop:"Stopknop tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="covers")return"Melden ze hun stand terug, dan komt er vanzelf een schuif bij. Zo niet, dan blijven het open, stop en dicht, en volgt het icoon de knop die je indrukt."}};j("domotiapp-cover-card-editor",Te);S("domotiapp-cover-card",Ne,{name:"DomotiApp Rolluiken",description:"Open, stop en dicht, met een eigen icoon voor open en dicht."});function Et(i){if(!i)return{label:"Onbekend",home:null};switch(i.state){case"home":return{label:"Thuis",home:!0};case"not_home":return{label:"Afwezig",home:!1};case"unknown":case"unavailable":return{label:"Onbekend",home:null};default:return{label:i.state,home:!1}}}var De=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.persons??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,persons:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[O]:"Kies minstens \xE9\xE9n persoon."}}watched(){return this.config.persons.map(e=>e.entity)}template(){let e=this.config;e.bare&&this.setAttribute("bare","");let t=e.columns??Math.min(e.persons.length,6),n=e.persons.map((a,o)=>`
      <button class="p" type="button" data-i="${o}" style="--tone:var(--dac-ink-3)">
        ...
        <span class="nm"></span>
        <span class="st"></span>
      </button>`).join("");return`<div class="card surface"><div class="chips" style="--cols:${t}">${n}</div></div>`}wire(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i];this.teardown_.push(A(e,{onTap:()=>P(this,t.entity)}))})}paint(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i],n=f(this.hass,t.entity),a=Et(n);e.style.setProperty("--tone",a.home===!0?"var(--dac-good)":a.home===!1?"var(--dac-bad)":"var(--dac-warn)");let o=z(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),o),this.text(e.querySelector(".st"),a.label);let r=e.querySelector(".ph"),s=n?.attributes?.entity_picture,l=s?`img:${s}`:o?`ini:${o[0]}`:"icon";r.dataset.kind!==l&&(r.dataset.kind=l,r.innerHTML=s?`<img src="${s}" alt="" loading="lazy" />`:o?o[0].toUpperCase():x.person),e.setAttribute("aria-label",`${o}, ${a.label}`)})}rows_(){let e=this.config?.columns??Math.min(this.config?.persons?.length??1,6),t=Math.ceil((this.config?.persons?.length??1)/e);return H(20+t*74+(t-1)*6)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:"full",rows:e,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-person-card-editor")}static getStubConfig(e){return{persons:Object.keys(e?.states??{}).filter(n=>n.startsWith("person.")).slice(0,6)}}},Ie=class extends E{setConfig(e){let t={...e},n=(e.persons??[]).map(a=>typeof a=="string"?{entity:a}:a);t.persons=n.map(a=>a.entity);for(let a of n)a.name&&(t[`naam:${a.entity}`]=a.name);super.setConfig(t)}serialize(e){let t={...e},n=t.persons??[];t.persons=n.map(a=>{let o=t[`naam:${a}`];return o?{entity:a,name:o}:a});for(let a of Object.keys(t))a.startsWith("naam:")&&delete t[a];return t}schema(){let e=(this.config_?.persons??[]).filter(t=>typeof t=="string");return[{name:"persons",selector:{entity:{domain:["person","device_tracker"],multiple:!0}}},...e.map(t=>({name:`naam:${t}`,selector:g.text()}))]}label(e){if(e.name==="persons")return"Personen";if(e.name.startsWith("naam:")){let t=e.name.slice(5);return`Naam voor ${this.hass?.states?.[t]?.attributes?.friendly_name??t}`}return super.label(e)}helper(e){if(e.name==="persons")return"Thuis is groen, weg is rood, geen melding is oranje. Per persoon kun je hieronder een eigen naam zetten."}};j("domotiapp-person-card-editor",Ie);S("domotiapp-person-card",De,{name:"DomotiApp Personen",description:"Wie er thuis is, compact. Het hele huishouden in \xE9\xE9n kaart."});var zt=[[/gft|groente|tuin|organi/i,"teal","binWheeled"],[/pmd|plastic|verpakking/i,"solar","binWheeled"],[/papier|karton/i,"water","binWheeled"],[/rest|grijs/i,"neutral","binWheeled"],[/textiel|kleding/i,"pink","bin"],[/glas/i,"magenta","bin"],[/kerstboom|snoei|takken/i,"teal","bin"]];function Ct(i){for(let[e,t,n]of zt)if(e.test(i))return{tone:t,icon:n};return{tone:"accent",icon:"bin"}}var nt=i=>String(i??"").replace(/^(afvalbeheer|afvalwijzer|mijnafvalwijzer)\s*/i,"").replace(/\s*(mijnafvalwijzer)\s*/i," ").trim(),Pe=class extends _{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.sensors??e.entities??(e.entity?[e.entity]:[]);return t.length?{show_hero:!0,show_list:!0,...e,sensors:t.map(n=>typeof n=="string"?{entity:n}:n)}:{...e,[O]:"Kies minstens \xE9\xE9n afvalsensor waarvan de status een datum is."}}watched(){return this.config.sensors.map(e=>e.entity)}read_(){let e=new Date;return this.config.sensors.map(t=>{let n=f(this.hass,t.entity);if(!n)return null;let a=G(n.state)??G(n.attributes.date)??G(n.attributes.next_date);if(!a)return null;let o=t.label??nt(z(this.hass,t.entity,t.name)),r=Ct(t.label??t.entity+o),s=this.config.tones?.[t.entity];return{label:o,date:a,days:le(e,a),tone:C(s??t.tone??r.tone),icon:t.icon??r.icon}}).filter(t=>t&&t.days>=0).sort((t,n)=>t.date-n.date)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),`
      ...
      </div>`}paint(){let e=this.read_(),t=this.$(".hero"),n=this.$(".list"),a=this.$(".empty");if(a.hidden=e.length>0,t&&(t.hidden=e.length===0,e.length)){let o=e[0];t.style.setProperty("--tone",o.tone),this.setAttribute("urgency",o.days===0?"today":o.days===1?"tomorrow":"later");let r=t.querySelector(".bin");r.dataset.icon!==o.icon&&(r.dataset.icon=o.icon,r.innerHTML=$(o.icon,"bin")),this.text(t.querySelector(".eyebrow"),de(o.date)),this.text(t.querySelector(".big"),o.label),this.text(t.querySelector(".n"),o.days===0?"nu":String(o.days)),this.text(t.querySelector(".u"),o.days===0?"aan de weg":o.days===1?"dag":"dagen")}if(n){let o=this.config.show_hero===!1?e:e.slice(1),r=o.map(s=>`${s.label}${+s.date}`).join("|");if(n.dataset.sig===r)return;n.dataset.sig=r,n.innerHTML=o.map(s=>{let l=de(s.date),h=s.days<=6?`<small>${Be(s.date)}</small>`:"";return`
        <div class="r" style="--tone:${s.tone}">
          <i></i><span>${s.label}</span>
          <span class="d">${l}${h}</span>
        </div>`}).join("")}}rows_(){let e=this.config?.sensors?.length??1;return this.config?.show_list===!1?1:this.config?.show_hero===!1?Math.max(1,H(20+e*33)):Math.max(2,e)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-waste-card-editor")}static getStubConfig(e){return{sensors:Object.keys(e?.states??{}).filter(n=>/afval|waste|trash|garbage|ophaal/i.test(n)&&n.startsWith("sensor.")).filter(n=>G(e.states[n]?.state)).slice(0,6),title:"Afvalkalender"}}},He=class extends E{defaults(){return{show_hero:!0,show_list:!0}}setConfig(e){let t={...e};for(let[n,a]of Object.entries(e.tones??{}))t[`kleur:${n}`]=a;delete t.tones,super.setConfig(t)}serialize(e){let t={...e},n={};for(let a of Object.keys(t))a.startsWith("kleur:")&&(t[a]&&(n[a.slice(6)]=t[a]),delete t[a]);return Object.keys(n).length?t.tones=n:delete t.tones,t}ids_(){return(this.config_?.sensors??[]).map(e=>typeof e=="string"?e:e.entity).filter(Boolean)}pickers(){return this.ids_().map(e=>({key:`kleur:${e}`,kind:"tone",label:`Kleur voor ${nt(this.hass?.states?.[e]?.attributes?.friendly_name??e)||e}`,compact:!0,after:!0}))}schema(){return[{name:"sensors",selector:{entity:{domain:"sensor",multiple:!0}}}]}label(e){return{sensors:"Afvalsensoren",show_hero:"Eerstvolgende uitlichten",show_list:"Overige data tonen",bare:"Zonder kaartrand"}[e.name]??super.label(e)}helper(e){if(e.name==="sensors")return"Sensoren waarvan de status een datum is, bijvoorbeeld 18-08-2026. De kaart sorteert zelf; laat een kleur leeg om de bakkleur op de naam te laten kiezen."}};j("domotiapp-waste-card-editor",He);S("domotiapp-waste-card",Pe,{name:"DomotiApp Afvalkalender",description:"Eerstvolgende ophaling als hero, de rest eronder. Kleur per fractie."});var jt="0.1.11";console.info(`%c DOMOTIAPP-CARDS %c ${jt} `,"background:#026fa1;color:#e8e4de;font-weight:600;border-radius:3px 0 0 3px;padding:2px 6px","background:#12120f;color:#e8e4de;border-radius:0 3px 3px 0;padding:2px 6px");export{jt as VERSION};
