/*! DomotiApp Cards 0.1.4 | MIT | https://github.com/Sven2410/domotiapp-cards */
var j=`
  --dac-bg:            #0c0c0a;
  --dac-bg-raise:      #12120f;
...
`;function T(n){let e=new CSSStyleSheet;return e.replaceSync(n),e}var $=n=>String(n??"").split(".")[0],p=(n,e)=>e&&n?.states?.[e]||null,S=(n,e)=>p(n,e)?.attributes??{};function y(n,e,t){return t||S(n,e).friendly_name||e||""}var Ae=new Set(["scene","script","input_button","button","event"]),B=n=>Ae.has($(n));function V(n){return!n||n.state==="unavailable"?!0:n.state==="unknown"?!B(n.entity_id):!1}function ye(n){if(!n)return!1;let e=n.state;if(e==="unavailable"||e==="unknown")return!1;switch($(n.entity_id)){case"cover":return e==="open"||e==="opening";case"alarm_control_panel":return e.startsWith("armed")||e==="triggered"||e==="arming";case"climate":case"water_heater":case"humidifier":return e!=="off";case"person":case"device_tracker":return e==="home";case"media_player":return e!=="off"&&e!=="idle"&&e!=="standby";default:return e==="on"||e==="playing"||e==="active"||e==="heat"}}function ke(n,e,t){if(!n||n.themes!==e.themes||n.language!==e.language)return!0;for(let a of t)if(a&&n.states?.[a]!==e.states?.[a])return!0;return!1}function W(n,e,t={}){n.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0,cancelable:!1}))}var O=(n,e)=>W(n,"hass-more-info",{entityId:e});function _e(n){switch($(n)){case"light":case"switch":case"fan":case"input_boolean":case"automation":case"siren":return{action:"toggle"};case"script":case"scene":case"input_button":case"button":return{action:"toggle"};default:return{action:"more-info"}}}function De(n){switch($(n)){case"scene":return["scene","turn_on"];case"script":return["script","turn_on"];case"input_button":return["input_button","press"];case"button":return["button","press"];case"lock":return["lock","open"];case"cover":return["cover","toggle"];case"media_player":return["media_player","media_play_pause"];default:return["homeassistant","toggle"]}}function Me(n,e,t,a){if(!(!a||a.action==="none"))switch(a.action){case"more-info":O(n,a.entity||t.entity);break;case"toggle":{let i=a.entity||t.entity;if(!i)break;let[r,s]=De(i);e.callService(r,s,{entity_id:i});break}case"perform-action":case"call-service":{let i=a.perform_action||a.service;if(!i)break;let[r,s]=i.split(".");e.callService(r,s,a.data??a.service_data??{},a.target);break}case"navigate":if(!a.navigation_path)break;history.pushState(null,"",a.navigation_path),W(window,"location-changed",{replace:!1});break;case"url":a.url_path&&window.open(a.url_path,a.target??"_blank");break;case"assist":W(n,"show-dialog",{dialogTag:"ha-voice-command-dialog",dialogImport:()=>{},dialogParams:{}});break;case"fire-dom-event":W(n,"ll-custom",a);break;default:break}}function E(n,{onTap:e,onHold:t,onDouble:a}){let s=0,c=0,l=null,h=v=>{v.button!=null&&v.button!==0||(s=Date.now())},m=()=>{let v=s?Date.now()-s:0;if(s=0,t&&v>=500){navigator.vibrate?.(18),t();return}if(!a){e?.();return}if(c++,c===1){l=setTimeout(()=>{c=0,e?.()},260);return}clearTimeout(l),c=0,a()};return n.addEventListener("pointerdown",h),n.addEventListener("click",m),n.addEventListener("contextmenu",v=>v.preventDefault()),()=>{clearTimeout(l),n.removeEventListener("pointerdown",h),n.removeEventListener("click",m)}}function C(n,e){if(!e)return"";let t=$(e.entity_id),a=e.attributes.device_class;return n.formatEntityState?.(e)??n.localize?.(`component.${t}.entity_component.${a??"_"}.state.${e.state}`)??n.localize?.(`component.${t}.entity_component._.state.${e.state}`)??e.state}function L(n,e,t){let a=Number(e);return Number.isFinite(a)?a.toLocaleString(n?.locale?.language??"nl",{minimumFractionDigits:t??0,maximumFractionDigits:t??0}):"--"}var we=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],$e=["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"],xe=(n=new Date)=>new Date(n.getFullYear(),n.getMonth(),n.getDate()),U=(n,e)=>Math.round((xe(e)-xe(n))/864e5);function H(n){if(!n)return null;if(n instanceof Date)return Number.isNaN(+n)?null:n;let e=String(n).trim(),t=e.match(/^(\d{1,2})-(\d{1,2})-(\d{4})/);if(t)return new Date(+t[3],+t[2]-1,+t[1]);if(t=e.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/),t)return new Date(+t[1],+t[2]-1,+t[3]);let a=new Date(e);return Number.isNaN(+a)?null:a}function Z(n,e=new Date){if(!n)return"";let t=U(e,n);return t<0?`${Math.abs(t)} dagen geleden`:t===0?"vandaag":t===1?"morgen":t===2?"overmorgen":t<=6?we[n.getDay()]:`${we[n.getDay()].slice(0,2)} ${n.getDate()} ${$e[n.getMonth()]}`}var Se=n=>n?`${n.getDate()} ${$e[n.getMonth()]}`:"";var je=`
  :host {
    ${j}
    display: block;
...
`,M={accent:"var(--dac-accent-hi)",solar:"var(--dac-solar)",house:"var(--dac-house)",water:"var(--dac-grid-in)",magenta:"var(--dac-grid-out)",pink:"var(--dac-device-1)",teal:"var(--dac-device-2)",lit:"var(--dac-lit)",good:"var(--dac-good)",warn:"var(--dac-warn)",bad:"var(--dac-bad)",neutral:"var(--dac-ink-3)"},N={accent:"Accent",solar:"Oranje",house:"Blauw",water:"Lichtblauw",magenta:"Magenta",pink:"Roze",teal:"Groenblauw",lit:"Lampgeel",good:"Goed",warn:"Let op",bad:"Kritiek",neutral:"Neutraal"},k=(n,e="accent")=>M[n]??(n&&/[#(]|^var/.test(n)?n:M[e]),_=Symbol("incomplete"),Te=n=>`
  <div class="needs">
  ...
  </div>`,Ne=56,Ee=8,q=n=>Math.max(1,Math.ceil((n+Ee)/(Ne+Ee))),g=class extends HTMLElement{static css="";static get styleSheets_(){return Object.hasOwn(this,"sheets_")||(this.sheets_=[T(je+ve+this.css)]),this.sheets_}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.adoptedStyleSheets=new.target.styleSheets_,this.built_=!1,this.wired_=!1,this.teardown_=[]}setConfig(e){let t=this.validate(e??{});this.config=t,this.built_&&(this.destroy_(),this.shadowRoot.replaceChildren(),this.built_=!1,this.wired_=!1),this.isConnected&&this.build_()}set hass(e){let t=this.hass_;if(this.hass_=e,!!this.config){if(!this.built_){this.build_();return}this.config[_]||ke(t,e,this.watched())&&this.paint()}}get hass(){return this.hass_}connectedCallback(){if(this.config){if(!this.built_){this.build_();return}this.wired_||(this.wire(),this.wired_=!0,this.hass_&&this.paint())}}disconnectedCallback(){this.destroy_(),this.wired_=!1}validate(e){return e}watched(){return this.config?.entity?[this.config.entity]:[]}template(){return""}wire(){}paint(){}build_(){let e=document.createElement("template"),t=this.config?.[_];e.innerHTML=t?Te(t):this.template(),this.shadowRoot.appendChild(e.content),this.built_=!0,!t&&(this.wire(),this.wired_=!0,this.hass_&&this.paint())}destroy_(){for(let e of this.teardown_)try{e()}catch{}this.teardown_=[]}$(e){return this.shadowRoot.querySelector(e)}$$(e){return[...this.shadowRoot.querySelectorAll(e)]}text(e,t){let a=typeof e=="string"?this.$(e):e;a&&a.textContent!==String(t)&&(a.textContent=t)}getCardSize(){return 1}};function w(n,e,{name:t,description:a,preview:i=!0}={}){customElements.get(n)||(customElements.define(n,e),window.customCards=window.customCards??[],window.customCards.push({type:n,name:t??n,description:a??"",preview:i,documentationURL:"https://github.com/Sven2410/domotiapp-cards"}))}function x(n,e){customElements.get(n)||customElements.define(n,e)}var o=(n,e="none")=>`<svg class="icon" viewBox="0 0 24 24" fill="${e}" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${n}</svg>`,u={house:o(`<path d="M3.2 11.3 12 4.1l8.8 7.2"/>
...
`,F=null,Y=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),F=F??[T(He)],this.shadowRoot.adoptedStyleSheets=F,this.value_="",this.label="Icoon",this.fallback="question",this.auto=!0}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}connectedCallback(){this.built_||(this.built_=!0,this.build_())}build_(){this.shadowRoot.innerHTML=`
        ...
        <div class="panel">
          ${qe.map(([t,a])=>`
            <div class="group">
              <h4>${t}</h4>
              <div class="grid">
                ${a.map(i=>`<button type="button" class="opt" data-icon="${i}" title="${i}" aria-pressed="false">${u[i]??""}</button>`).join("")}
              </div>
            </div>`).join("")}
          <div class="mdi">
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  :focus-visible { outline: 2px solid var(--dac-accent-hi); outline-offset: 2px; }
`,X=null,J=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),X=X??[T(Pe)],this.shadowRoot.adoptedStyleSheets=X,this.value_="",this.label="Kleur",this.statuses=!1}set value(e){this.value_=e??"",this.built_&&this.paint_()}get value(){return this.value_}connectedCallback(){this.built_||(this.built_=!0,this.build_())}swatch(e){return`<button type="button" class="sw" data-tone="${e}" style="--c:${M[e]}"
      title="${N[e]}" aria-label="${N[e]}" aria-pressed="false">${u.check}</button>`}build_(){this.shadowRoot.innerHTML=`
      <div class="label"></div>
      <div class="box">
        ...
        <div class="row">
          ...
          ${Re.map(e=>this.swatch(e)).join("")}
        </div>
        ${this.statuses?`<h4>Status</h4>
               <div class="row">${Ie.map(e=>this.swatch(e)).join("")}</div>
               <p class="note">
      ...
      </div>`,this.shadowRoot.querySelectorAll(".sw").forEach(e=>e.addEventListener("click",()=>this.emit_(e.dataset.tone))),this.paint_()}paint_(){this.shadowRoot.firstElementChild&&(this.$(".label").textContent=this.label??"Kleur",this.shadowRoot.querySelectorAll(".sw").forEach(e=>e.setAttribute("aria-pressed",String((e.dataset.tone||"")===this.value_))),this.$(".chosen").innerHTML=this.value_?`Gekozen: <b>${N[this.value_]??this.value_}</b>`:"Gekozen: <b>Automatisch</b> &mdash; de kaart kiest op domein en toestand.")}emit_(e){this.value_=e??"",this.paint_(),this.dispatchEvent(new CustomEvent("value-changed",{detail:{value:this.value_},bubbles:!0,composed:!0}))}$(e){return this.shadowRoot.querySelector(e)}};customElements.get("dac-tone-picker")||customElements.define("dac-tone-picker",J);var d={entity:n=>({entity:n?{domain:n}:{}}),text:()=>({text:{}}),multiline:()=>({text:{multiline:!0}}),bool:()=>({boolean:{}}),number:(n,e,t=1)=>({number:{min:n,max:e,step:t,mode:"box"}}),select:n=>({select:{mode:"dropdown",options:n}}),action:()=>({ui_action:{}})},Q=(...n)=>({type:"grid",name:"",schema:n});var f=class extends HTMLElement{constructor(){super(),this.config_={},this.built_=!1}setConfig(e){this.config_={...this.defaults(),...e},this.render_()}defaults(){return{}}set hass(e){this.hass_=e,this.form_&&(this.form_.hass=e);for(let t of this.pickers_??[])t.hass=e;this.render_()}get hass(){return this.hass_}connectedCallback(){this.render_()}schema(){return[]}pickers(){return[]}label(e){return ee[e.name]??e.name}helper(){}async render_(){if(!this.hass_||!this.config_)return;if(this.built_){this.sync_();return}this.built_=!0,await customElements.whenDefined("ha-form"),this.replaceChildren(),this.pickers_=[];let e=this.pickers();if(e.length){let a=document.createElement("div");a.style.cssText="display:flex;flex-direction:column;gap:12px;margin-bottom:16px";for(let i of e){let r=document.createElement(i.kind==="tone"?"dac-tone-picker":"dac-icon-picker");r.label=i.label,r.fallback=i.fallback,i.auto===!1&&(r.auto=!1),i.statuses===!1&&(r.statuses=!1),r.hass=this.hass_,r.value=this.config_[i.key],r.addEventListener("value-changed",s=>{s.stopPropagation(),this.patch_({[i.key]:s.detail.value})}),this.pickers_.push(r),r.dataset.key=i.key,a.appendChild(r)}this.appendChild(a)}let t=document.createElement("ha-form");t.hass=this.hass_,t.data=this.config_,t.schema=this.schema(),t.computeLabel=a=>this.label(a),t.computeHelper=a=>this.helper(a),t.addEventListener("value-changed",a=>{a.stopPropagation(),this.patch_(a.detail.value,!0)}),this.form_=t,this.appendChild(t)}sync_(){this.form_&&(this.form_.hass=this.hass_,this.form_.schema=this.schema(),this.form_.data=this.config_);for(let e of this.pickers_??[])e.hass=this.hass_,e.value=this.config_[e.dataset.key]}patch_(e,t=!1){let a=t?{...e}:{...this.config_,...e};this.config_.type&&(a.type=this.config_.type);for(let[i,r]of Object.entries(a))(r===""||r===void 0||r===null)&&delete a[i];this.config_=a,this.sync_(),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this.serialize(a)},bubbles:!0,composed:!0}))}serialize(e){return e}},ee={entity:"Entiteit",entities:"Entiteiten",name:"Naam",icon:"Icoon",tone:"Kleur",secondary:"Tweede regel",layout:"Vorm",tap_action:"Tikken",hold_action:"Vasthouden",double_tap_action:"Dubbeltikken",show_state:"Toestand tonen",show_name:"Naam tonen",show_icon:"Icoon tonen",state_color:"Kleur volgt toestand",fill:"Vullen",collapsible:"Inklapbaar",title:"Titel",subtitle:"Ondertitel",weather:"Weerentiteit",sun:"Zon-entiteit",person:"Persoon",persons:"Personen",covers:"Rolluiken",lights:"Lampen",sensors:"Sensoren",greeting:"Begroeting",show_clock:"Klok tonen",show_weather:"Weer tonen",show_chips:"Weerdetails tonen",compact:"Compact",columns:"Kolommen",group:"Groepsregel tonen",invert:"Open en dicht omdraaien",label:"Label",color:"Kleur",date_format:"Datumnotatie"};function We(n=new Date){let e=n.getHours();return e<6?"Goedenacht":e<12?"Goedemorgen":e<18?"Goedemiddag":"Goedenavond"}var Ve=["zondag","maandag","dinsdag","woensdag","donderdag","vrijdag","zaterdag"],Ke=["januari","februari","maart","april","mei","juni","juli","augustus","september","oktober","november","december"],te={humidity:{icon:"drop",tone:"water",label:"Luchtvochtigheid"},wind:{icon:"wind",tone:"neutral",label:"Wind"},uv:{icon:"uv",tone:"solar",label:"UV-index"},precipitation:{icon:"rain",tone:"water",label:"Neerslag"},pressure:{icon:"gaugeArrow",tone:"neutral",label:"Luchtdruk"},sunrise:{icon:"sunrise",tone:"warn",label:"Zonsopkomst"},sunset:{icon:"sunset",tone:"warn",label:"Zonsondergang"}},Ge=["humidity","wind","uv","precipitation","sunset"],Be=n=>n==null||Number.isNaN(+n)?"":["N","NO","O","ZO","Z","ZW","W","NW"][Math.round(+n/45)%8],ae=class extends g{static css=`
    ...
    /* E\xE9n regel, en wat er niet op past wordt hele chips weggelaten.
       Wrappen leek netter tot het de strip twee regels hoog maakte terwijl hij
       er \xE9\xE9n had opgegeven bij het raster: dan loopt de kaart onder de volgende
  ...
  `;validate(e){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768,...e}}watched(){let e=this.config;return[e.weather,e.weather_uv,e.sun,e.precipitation_entity].filter(Boolean)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),e.show_rule===!1&&this.setAttribute("no-rule",""),`
      ...
      </div>`}wire(){let e=()=>{let a=6e4-Date.now()%6e4+50;this.timer_=setTimeout(()=>{this.paintClock_(),e()},a)};e(),this.teardown_.push(()=>clearTimeout(this.timer_));let t=Number(this.config.hide_below)||0;if(t>0){let a=matchMedia(`(max-width: ${t-1}px)`),i=()=>this.toggleAttribute("narrow",a.matches);i(),a.addEventListener("change",i),this.teardown_.push(()=>a.removeEventListener("change",i))}}paintClock_(){let e=new Date,t=this.config.name??this.hass?.user?.name??"",a=We(e);this.$(".hello").innerHTML=t?`${a}, <b>${t}</b>`:a,this.text(".date",`${Ve[e.getDay()]} ${e.getDate()} ${Ke[e.getMonth()]}`);let i=this.$(".clock");i&&this.text(i,e.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"}))}paint(){this.paintClock_();let e=this.config,t=p(this.hass,e.weather),a=S(this.hass,e.weather),i=this.$(".now");if(i&&t){let l=Oe(t.state);i.style.setProperty("--wtone",k(e.tone,"water"));let h=this.hass?.config?.unit_system?.temperature??"\xB0C";this.$(".temp").innerHTML=a.temperature!=null?`${L(this.hass,a.temperature,0)}<span>${h}</span>`:"--";let m=i.querySelector(".ic");m.dataset.icon!==l&&(m.dataset.icon=l,m.innerHTML=b(l,"cloud")),this.text(i.querySelector(".cond"),C(this.hass,t))}let r=this.$(".chips");if(!r)return;let s=Ge.map(l=>this.chip_(l,a)).filter(Boolean),c=s.map(l=>`${l.key}${l.value}`).join("|");r.dataset.sig!==c&&(r.dataset.sig=c,r.innerHTML=s.map(l=>`<span class="chip2" style="--tone:${k(te[l.key].tone)}" title="${te[l.key].label}">
             ${u[te[l.key].icon]??""}${l.value}
           </span>`).join(""))}chip_(e,t){let a=this.config;switch(e){case"humidity":return t.humidity!=null?{key:e,value:`${Math.round(t.humidity)}%`}:null;case"wind":{if(t.wind_speed==null)return null;let i=this.hass?.config?.unit_system?.wind_speed??"km/h",r=Be(t.wind_bearing);return{key:e,value:`${L(this.hass,t.wind_speed,0)} ${i}${r?` ${r}`:""}`}}case"uv":{let r=S(this.hass,a.weather_uv).uv_index??t.uv_index??(a.weather_uv?Number(p(this.hass,a.weather_uv)?.state):null);return r!=null&&!Number.isNaN(+r)?{key:e,value:`UV ${L(this.hass,r,1)}`}:null}case"precipitation":{let i=p(this.hass,a.precipitation_entity);if(i){let r=Number(i.state);if(Number.isNaN(r))return null;let s=i.attributes.unit_of_measurement??"mm";return{key:e,value:`${L(this.hass,r,1)} ${s}`}}return t.precipitation!=null&&!Number.isNaN(+t.precipitation)?{key:e,value:`${L(this.hass,t.precipitation,1)} mm`}:null}case"pressure":return t.pressure!=null?{key:e,value:`${L(this.hass,t.pressure,0)} ${t.pressure_unit??"hPa"}`}:null;case"sunset":case"sunrise":{let r=p(this.hass,a.sun)?.attributes?.[e==="sunset"?"next_setting":"next_rising"];if(!r)return null;let s=new Date(r);return Number.isNaN(+s)?null:{key:e,value:s.toLocaleTimeString(this.hass?.locale?.language??"nl",{hour:"2-digit",minute:"2-digit"})}}default:return null}}getCardSize(){return 1}getGridOptions(){return{columns:"full",rows:1,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-header-card-editor")}static getStubConfig(e){return{weather:Object.keys(e?.states??{}).find(a=>a.startsWith("weather.")),sun:"sun.sun"}}},ne=class extends f{defaults(){return{show_clock:!0,show_weather:!0,show_chips:!0,show_rule:!0,hide_below:768}}pickers(){return[{key:"tone",kind:"tone",label:"Kleur weericoon"}]}schema(){return[Q({name:"weather",selector:d.entity("weather")},{name:"weather_uv",selector:{entity:{domain:["weather","sensor"]}}}),Q({name:"sun",selector:d.entity("sun")},{name:"precipitation_entity",selector:d.entity("sensor")}),{name:"name",selector:d.text()},{name:"hide_below",selector:d.number(0,1400,8)}]}label(e){return{weather:"Weer (temperatuur, wind)",weather_uv:"Tweede weerbron (UV-index)",precipitation_entity:"Neerslagsensor",show_rule:"Accentlijn tonen",hide_below:"Verbergen onder breedte (px)",bare:"Zonder kaartrand",name:"Naam"}[e.name]??super.label(e)}helper(e){if(e.name==="weather_uv")return"Alleen voor de UV-index. Handig als je hoofdbron die niet meelevert.";if(e.name==="precipitation_entity")return"Een sensor in mm of mm/h, bijvoorbeeld neerslagintensiteit of regen laatste uur.";if(e.name==="hide_below")return"768 verbergt de header op telefoons en houdt hem op tablets en desktops. 0 zet het uit.";if(e.name==="name")return"Leeg laten voor de naam van de ingelogde gebruiker."}};x("domotiapp-header-card-editor",ne);w("domotiapp-header-card",ae,{name:"DomotiApp Header",description:"Smalle strip met begroeting, weer en klok. Verbergt zichzelf op telefoons."});var ie=class extends g{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return{icon:"",tone:"accent",line:!0,...e}}watched(){return this.config.secondary_entity?[this.config.secondary_entity]:[]}template(){let e=this.config,t=e.icon!==null&&e.icon!==!1;return t||this.setAttribute("no-icon",""),`
      ...
      </div>`}paint(){this.text("h3",this.config.name??"");let e=this.$(".sub");if(!e)return;let t=p(this.hass,this.config.secondary_entity),a=e.querySelector(".si"),i=e.querySelector(".sv");if(!t){i.textContent="",a.innerHTML="";return}let r=this.config.secondary_icon??"";a.dataset.icon!==r&&(a.dataset.icon=r,a.innerHTML=r?b(r):"");let s=t.attributes.unit_of_measurement;i.textContent=s?`${t.state} ${s}`:t.attributes.current_temperature!=null?`${t.attributes.current_temperature} \xB0C`:C(this.hass,t)}getCardSize(){return 1}getGridOptions(){return{columns:"full",rows:1,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-separator-card-editor")}static getStubConfig(){return{name:"Nieuwe sectie",icon:"house",tone:"accent"}}},re=class extends f{defaults(){return{line:!0,tone:"accent"}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon links",fallback:"star",auto:!1},{key:"tone",kind:"tone",label:ee.tone},{key:"secondary_icon",kind:"icon",label:"Icoon bij de waarde rechts",auto:!1}]}schema(){return[{name:"name",selector:d.text()},{name:"line",selector:d.bool()},{name:"secondary_entity",selector:d.entity()}]}label(e){return{line:"Lijn tonen",secondary_entity:"Waarde rechts (optioneel)"}[e.name]??super.label(e)}helper(e){if(e.name==="secondary_entity")return"Toont de toestand van deze entiteit rechts van de lijn, bijvoorbeeld een temperatuur of een aantal."}};x("domotiapp-separator-card-editor",re);w("domotiapp-separator-card",ie,{name:"DomotiApp Separator",description:"Sectiekop met icoon en vervagende lijn."});var oe=class extends g{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){return{layout:"row",state_color:!0,show_state:!0,show_name:!0,show_icon:!0,...e}}watched(){return[this.config.entity].filter(Boolean)}tone_(){let e=this.config;return e.tone?k(e.tone):$(e.entity)==="light"?M.lit:M.accent}template(){let e=this.config;return this.setAttribute("layout",["row","tile","compact"].includes(e.layout)?e.layout:"row"),`
      ...
      </button>`}wire(){let e=this.config,t=this.$(".btn"),a=(i,r)=>Me(this,this.hass,e,e[i]??r);this.teardown_.push(E(t,{onTap:()=>a("tap_action",_e(e.entity)),onHold:()=>a("hold_action",{action:e.entity?"more-info":"none"}),onDouble:e.double_tap_action?()=>a("double_tap_action",{action:"none"}):void 0}))}paint(){let e=this.config,t=p(this.hass,e.entity),a=e.state_color!==!1&&ye(t),i=!!e.entity&&V(t);this.toggleAttribute("on",a),this.$(".btn").classList.toggle("unavailable",i);let r=this.$(".chip");if(r){let c=e.icon||ze(e.entity,S(this.hass,e.entity));r.dataset.icon!==c&&(r.dataset.icon=c,r.innerHTML=b(c)),r.style.setProperty("--tone",a?this.tone_():"var(--dac-ink-3)")}this.text(".nm",y(this.hass,e.entity,e.name));let s=this.$(".st");s&&this.text(s,this.secondary_(t,i)),this.$(".btn").setAttribute("aria-label",`${y(this.hass,e.entity,e.name)}${t?`, ${C(this.hass,t)}`:""}`)}secondary_(e,t){return t?"Niet bereikbaar":!e||B(e.entity_id)?"":$(e.entity_id)==="light"&&e.state==="on"&&e.attributes.brightness!=null?`${Math.round(e.attributes.brightness/255*100)}%`:C(this.hass,e)}getCardSize(){return this.config?.layout==="tile"?2:1}getGridOptions(){return this.config?.layout==="tile"?{columns:6,rows:2,min_columns:3,min_rows:2,max_rows:2}:{columns:12,rows:1,min_columns:4,min_rows:1,max_rows:1}}static getConfigElement(){return document.createElement("domotiapp-button-card-editor")}static getStubConfig(e,t){return{entity:t?.find(i=>i.startsWith("light."))??t?.find(i=>i.startsWith("switch."))??t?.[0],layout:"row"}}},se=class extends f{defaults(){return{layout:"row",state_color:!0,show_state:!0,show_name:!0,show_icon:!0}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"star"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"entity",selector:d.entity()},{name:"name",selector:d.text()},{name:"layout",selector:d.select([{value:"row",label:"Rij"},{value:"tile",label:"Tegel"},{value:"compact",label:"Compact"}])},{name:"show_icon",selector:d.bool()},{name:"show_name",selector:d.bool()},{name:"show_state",selector:d.bool()},{name:"state_color",selector:d.bool()},{name:"tap_action",selector:d.action()},{name:"hold_action",selector:d.action()},{name:"double_tap_action",selector:d.action()}]}label(e){return{entity:"Entiteit",name:"Naam (overschrijft die van de entiteit)",layout:"Vorm",show_icon:"Icoon tonen",show_name:"Naam tonen",show_state:"Toestand tonen",state_color:"Kleur volgt de toestand"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"Mag leeg blijven: zonder entiteit wordt dit een navigatieknop.";if(e.name==="tap_action")return"Leeg laten schakelt de entiteit, of opent meer informatie als schakelen niet kan."}};x("domotiapp-button-card-editor",se);w("domotiapp-button-card",oe,{name:"DomotiApp Knop",description:"E\xE9n control als rij, tegel of compacte pil. Vervangt tile, mushroom-entity en bubble-knoppen."});var Ue=new Set(["brightness","color_temp","hs","rgb","rgbw","rgbww","xy","white"]),Ze=new Set(["hs","rgb","rgbw","rgbww","xy"]),de=n=>n?.attributes?.supported_color_modes??[],Fe=n=>de(n).some(e=>Ue.has(e)),K=n=>de(n).some(e=>Ze.has(e)),G=n=>de(n).includes("color_temp"),Ye=n=>Math.max(1,Math.round((n??0)/255*100)),ce=class extends g{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.entity??e.lights?.[0]??e.entities?.[0],a=typeof t=="string"?t:t?.entity;return a?{show_colour:!0,...e,entity:a}:{...e,[_]:"Kies een lamp."}}watched(){return[this.config.entity]}template(){return this.config.bare&&this.setAttribute("bare",""),`
      <div class="card surface">
      ...
      </div>`}wire(){this.dragging_=new Set;let e=this.config.entity;this.teardown_.push(E(this.$(".chip"),{onTap:()=>this.hass.callService("light","toggle",{entity_id:e}),onHold:()=>O(this,e)}));let t=r=>{let s=r.target;if(s.type!=="range")return;let c=s.dataset.kind;this.dragging_.add(c);let l=+s.value,h=s.closest(".slider");c==="brightness"&&(h.style.setProperty("--v",`${l}%`),this.text(".v",l===0?"uit":`${l}%`))},a=r=>{let s=r.target;if(s.type!=="range")return;let c=s.dataset.kind;this.dragging_.delete(c);let l=+s.value;if(c==="brightness")l===0?this.hass.callService("light","turn_off",{entity_id:e}):this.hass.callService("light","turn_on",{entity_id:e,brightness_pct:l});else if(c==="hue"){let h=p(this.hass,e)?.attributes?.hs_color?.[1]??100;this.hass.callService("light","turn_on",{entity_id:e,hs_color:[l,h]})}else c==="kelvin"&&this.hass.callService("light","turn_on",{entity_id:e,color_temp_kelvin:l})},i=this.$(".card");i.addEventListener("input",t),i.addEventListener("change",a),i.addEventListener("click",r=>{r.target.closest(".toggle")&&this.hass.callService("light","toggle",{entity_id:e})})}paint(){let e=this.config,t=p(this.hass,e.entity),a=V(t),i=t?.state==="on",r=this.$(".lamp");r.dataset.on=String(i),r.classList.toggle("unavailable",a),this.toggleAttribute("lamp-off",!i);let s=this.$(".chip"),c=e.icon||"bulb";s.dataset.icon!==c&&(s.dataset.icon=c,s.innerHTML=b(c,"bulb")),this.text(".nm",y(this.hass,e.entity,e.name));let l=i?t?.attributes?.rgb_color:null;r.style.setProperty("--tone",l?`rgb(${l[0]},${l[1]},${l[2]})`:"var(--dac-lit)");let h=this.$(".ctl"),m=a?"none":Fe(t)?"range":"toggle";if(h.dataset.kind!==m&&(h.dataset.kind=m,h.innerHTML=m==="range"?`<span class="slider" style="--v:0%">
               <span class="track"><span class="fill"></span></span>
               <input type="range" data-kind="brightness" min="0" max="100" step="1" value="0"
                      aria-label="Helderheid" />
             </span>`:m==="toggle"?'<button class="toggle" type="button" role="switch" aria-checked="false" aria-label="Aan of uit"></button>':""),m==="range"&&!this.dragging_.has("brightness")){let v=i?Ye(t.attributes.brightness):0,A=h.querySelector("input");this.shadowRoot.activeElement!==A&&(A.value=String(v)),h.querySelector(".slider").style.setProperty("--v",`${v}%`),this.text(".v",i?`${v}%`:"uit")}else m==="toggle"?(h.querySelector(".toggle")?.setAttribute("aria-checked",String(i)),this.text(".v",i?"aan":"uit")):m==="none"&&this.text(".v","niet bereikbaar");this.paintColour_(t)}paintColour_(e){let t=this.$(".colour"),a=this.config.show_colour!==!1&&(K(e)||G(e));if(t.hidden=!a,!a)return;let i=`${K(e)?"c":""}${G(e)?"t":""}`;if(t.dataset.sig!==i){t.dataset.sig=i;let c=e.attributes.min_color_temp_kelvin??2e3,l=e.attributes.max_color_temp_kelvin??6500;t.innerHTML=`
        ${K(e)?`<span class="slider" data-strip title="Kleur" style="--strip:linear-gradient(90deg,
        ...
        ${G(e)?`<span class="slider" data-strip title="Kleurtemperatuur"
               ...
               </span>`:""}`}let r=t.querySelector('[data-kind="hue"]');r&&!this.dragging_.has("hue")&&this.shadowRoot.activeElement!==r&&(r.value=String(Math.round(e.attributes.hs_color?.[0]??0)));let s=t.querySelector('[data-kind="kelvin"]');if(s&&!this.dragging_.has("kelvin")&&this.shadowRoot.activeElement!==s){let c=e.attributes.color_temp_kelvin;c!=null&&(s.value=String(c))}}rows_(){let e=p(this.hass,this.config?.entity);return this.config?.show_colour!==!1&&e&&(K(e)||G(e))?2:1}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:4,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-light-card-editor")}static getStubConfig(e,t){let a=t?.find(i=>i.startsWith("light."));return a?{entity:a}:{}}},le=class extends f{defaults(){return{show_colour:!0}}pickers(){return[{key:"icon",kind:"icon",label:"Icoon",fallback:"bulb"}]}schema(){return[{name:"entity",selector:d.entity("light")},{name:"name",selector:d.text()},{name:"show_colour",selector:d.bool()}]}label(e){return{entity:"Lamp",name:"Naam (overschrijft die van de lamp)",show_colour:"Kleurstrips tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="entity")return"E\xE9n lamp per kaart. Dimbaar krijgt een schuif, alleen schakelbaar een tuimelaar.";if(e.name==="show_colour")return"Kleur en kleurtemperatuur, als de lamp ze kan. De kaart wordt dan twee rijen hoog."}};x("domotiapp-light-card-editor",le);w("domotiapp-light-card",ce,{name:"DomotiApp Verlichting",description:"E\xE9n lamp op \xE9\xE9n rasterrij: dimmen, kleur en kleurtemperatuur."});var R={OPEN:1,CLOSE:2,SET_POSITION:4,STOP:8},I=(n,e)=>!!((n?.attributes?.supported_features??0)&e),Xe=(n={})=>{switch(n.device_class){case"garage":return{open:"garageOpen",closed:"garageClosed"};case"awning":case"blind":return{open:"awning",closed:"awning"};default:return{open:"shutterOpen",closed:"shutter"}}},he=class extends g{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.covers??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,covers:t.map(a=>typeof a=="string"?{entity:a}:a)}:{...e,[_]:"Kies minstens \xE9\xE9n rolluik of zonnescherm."}}watched(){return this.config.covers.map(e=>e.entity)}keysHtml(e){return`
      <div class="keys">
      ...
      </div>`}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),`<div class="card surface">${e.covers.map((a,i)=>`
      <div class="cv" data-i="${i}" data-shown="closed" style="--tone:${k(a.tone??e.tone,"solar")}">
      ...
      </div>`).join("")}</div>`}wire(){this.dragging_=new Set,this.assumed_=new Map,this.$$(".cv").forEach(e=>{let t=e.dataset.i;e.querySelectorAll(".keys button").forEach(r=>{r.addEventListener("click",()=>{let s=r.dataset.act,c={open:"open_cover",stop:"stop_cover",close:"close_cover"};this.hass.callService("cover",c[s],{entity_id:this.config.covers[+t].entity}),s!=="stop"&&(this.assumed_.set(t,s==="open"?"open":"closed"),this.paint())})});let a=this.config.covers[+t].entity;this.teardown_.push(E(e.querySelector(".chip"),{onTap:()=>O(this,a)}));let i=e.querySelector(".pos");i.addEventListener("input",r=>{if(r.target.type!=="range")return;this.dragging_.add(t);let s=+r.target.value;i.querySelector(".slider").style.setProperty("--v",`${s}%`),e.querySelector(".st").textContent=`${s}% open`}),i.addEventListener("change",r=>{r.target.type==="range"&&(this.dragging_.delete(t),this.hass.callService("cover","set_cover_position",{entity_id:a,position:+r.target.value}))})})}paint(){this.$$(".cv").forEach(e=>{let t=e.dataset.i,a=this.config.covers[+t],i=p(this.hass,a.entity),r=S(this.hass,a.entity),s=!i||i.state==="unavailable",c=i?.state??"unknown";e.classList.toggle("unavailable",s),e.querySelector(".nm").textContent=y(this.hass,a.entity,a.name);let l=I(i,R.SET_POSITION)&&r.current_position!=null,h=l?r.current_position>0?"open":"closed":c==="open"||c==="closed"?c:this.assumed_.get(t)??"closed";e.dataset.shown=h;let m=Xe(r),v=(h==="open"?a.icon_open:a.icon_closed)??(h==="open"?this.config.icon_open:this.config.icon_closed)??a.icon??m[h],A=e.querySelector(".chip");A.dataset.icon!==v&&(A.dataset.icon=v,A.innerHTML=b(v,m[h]));let Le=e.querySelector(".st");this.dragging_.has(t)||(Le.textContent=s?"Niet bereikbaar":c==="opening"?"Gaat open":c==="closing"?"Gaat dicht":l?`${r.current_position}% open`:c==="open"?"Open":c==="closed"?"Dicht":""),e.querySelectorAll(".keys button").forEach(z=>{if(z.dataset.act==="stop"){z.disabled=s||!I(i,R.STOP);return}let P=z.dataset.act==="open";z.disabled=s||(P?!I(i,R.OPEN):!I(i,R.CLOSE))});let D=e.querySelector(".pos"),be=l&&this.config.show_position!==!1;if(D.hidden=!be,be&&(D.dataset.built||(D.dataset.built="1",D.innerHTML=`
            <div class="slider" style="--v:0%">
              <span class="track"><span class="fill"></span></span>
              <input type="range" min="0" max="100" step="1" value="0" aria-label="Positie" />
            </div>`),!this.dragging_.has(t))){let z=r.current_position??0,P=D.querySelector("input");this.shadowRoot.activeElement!==P&&(P.value=String(z)),D.querySelector(".slider").style.setProperty("--v",`${z}%`)}})}rows_(){let e=this.config?.covers??[],t=e.some(a=>I(p(this.hass,a.entity),R.SET_POSITION));return q(12+Math.max(1,e.length)*42+(t?30:0))}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-cover-card-editor")}static getStubConfig(e,t){let a=t?.find(i=>i.startsWith("cover."));return{covers:a?[a]:[]}}},pe=class extends f{defaults(){return{show_stop:!0,show_position:!0}}pickers(){return[{key:"icon_open",kind:"icon",label:"Icoon als het open staat",fallback:"shutterOpen"},{key:"icon_closed",kind:"icon",label:"Icoon als het dicht is",fallback:"shutter"},{key:"tone",kind:"tone",label:"Kleur"}]}schema(){return[{name:"covers",selector:{entity:{domain:"cover",multiple:!0}}},{name:"show_stop",selector:d.bool()}]}label(e){return{covers:"Rolluiken",show_stop:"Stopknop tonen"}[e.name]??super.label(e)}helper(e){if(e.name==="covers")return"Melden ze hun stand terug, dan komt er vanzelf een schuif bij. Zo niet, dan blijven het open, stop en dicht, en volgt het icoon de knop die je indrukt."}};x("domotiapp-cover-card-editor",pe);w("domotiapp-cover-card",he,{name:"DomotiApp Rolluiken",description:"Open, stop en dicht, met een eigen icoon voor open en dicht."});function Je(n){if(!n)return{label:"Onbekend",home:null};switch(n.state){case"home":return{label:"Thuis",home:!0};case"not_home":return{label:"Afwezig",home:!1};case"unknown":case"unavailable":return{label:"Onbekend",home:null};default:return{label:n.state,home:!1}}}var ue=class extends g{static css=`
    :host { display: block; height: 100%; }
    ...
    :focus-visible { outline: 2px solid var(--dac-accent-hi); outline-offset: 2px; }
  `;validate(e){let t=e.persons??e.entities??(e.entity?[e.entity]:[]);return t.length?{...e,persons:t.map(a=>typeof a=="string"?{entity:a}:a)}:{...e,[_]:"Kies minstens \xE9\xE9n persoon."}}watched(){return this.config.persons.map(e=>e.entity)}template(){let e=this.config;e.bare&&this.setAttribute("bare","");let t=e.columns??Math.min(e.persons.length,6),a=e.persons.map((i,r)=>`
      <button class="p" type="button" data-i="${r}" style="--tone:var(--dac-ink-3)">
        <span class="av"><span class="ph"></span></span>
        <span class="nm"></span>
        <span class="st"></span>
      </button>`).join("");return`<div class="card surface"><div class="chips" style="--cols:${t}">${a}</div></div>`}wire(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i];this.teardown_.push(E(e,{onTap:()=>O(this,t.entity)}))})}paint(){this.$$(".p").forEach(e=>{let t=this.config.persons[+e.dataset.i],a=p(this.hass,t.entity),i=Je(a);e.style.setProperty("--tone",i.home===!0?"var(--dac-good)":i.home===!1?"var(--dac-bad)":"var(--dac-warn)");let r=y(this.hass,t.entity,t.name);this.text(e.querySelector(".nm"),r),this.text(e.querySelector(".st"),i.label);let s=e.querySelector(".ph"),c=a?.attributes?.entity_picture,l=c?`img:${c}`:r?`ini:${r[0]}`:"icon";s.dataset.kind!==l&&(s.dataset.kind=l,s.innerHTML=c?`<img src="${c}" alt="" loading="lazy" />`:r?r[0].toUpperCase():u.person),e.setAttribute("aria-label",`${r}, ${i.label}`)})}rows_(){let e=this.config?.columns??Math.min(this.config?.persons?.length??1,6),t=Math.ceil((this.config?.persons?.length??1)/e);return q(20+t*74+(t-1)*6)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:"full",rows:e,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-person-card-editor")}static getStubConfig(e){return{persons:Object.keys(e?.states??{}).filter(a=>a.startsWith("person.")).slice(0,6)}}},me=class extends f{setConfig(e){let t={...e},a=(e.persons??[]).map(i=>typeof i=="string"?{entity:i}:i);t.persons=a.map(i=>i.entity);for(let i of a)i.name&&(t[`naam:${i.entity}`]=i.name);super.setConfig(t)}serialize(e){let t={...e},a=t.persons??[];t.persons=a.map(i=>{let r=t[`naam:${i}`];return r?{entity:i,name:r}:i});for(let i of Object.keys(t))i.startsWith("naam:")&&delete t[i];return t}schema(){let e=(this.config_?.persons??[]).filter(t=>typeof t=="string");return[{name:"persons",selector:{entity:{domain:["person","device_tracker"],multiple:!0}}},...e.map(t=>({name:`naam:${t}`,selector:d.text()}))]}label(e){if(e.name==="persons")return"Personen";if(e.name.startsWith("naam:")){let t=e.name.slice(5);return`Naam voor ${this.hass?.states?.[t]?.attributes?.friendly_name??t}`}return super.label(e)}helper(e){if(e.name==="persons")return"Thuis is groen, weg is rood, geen melding is oranje. Per persoon kun je hieronder een eigen naam zetten."}};x("domotiapp-person-card-editor",me);w("domotiapp-person-card",ue,{name:"DomotiApp Personen",description:"Wie er thuis is, compact. Het hele huishouden in \xE9\xE9n kaart."});var Qe=[[/gft|groente|tuin|organi/i,"teal","binWheeled"],[/pmd|plastic|verpakking/i,"solar","binWheeled"],[/papier|karton/i,"water","binWheeled"],[/rest|grijs/i,"neutral","binWheeled"],[/textiel|kleding/i,"pink","bin"],[/glas/i,"magenta","bin"],[/kerstboom|snoei|takken/i,"teal","bin"]];function et(n){for(let[e,t,a]of Qe)if(e.test(n))return{tone:t,icon:a};return{tone:"accent",icon:"bin"}}var Ce=n=>String(n??"").replace(/^(afvalbeheer|afvalwijzer|mijnafvalwijzer)\s*/i,"").replace(/\s*(mijnafvalwijzer)\s*/i," ").trim(),ge=class extends g{static css=`
    :host { display: block; height: 100%; }
  ...
  `;validate(e){let t=e.sensors??e.entities??(e.entity?[e.entity]:[]);return t.length?{show_hero:!0,show_list:!0,...e,sensors:t.map(a=>typeof a=="string"?{entity:a}:a)}:{...e,[_]:"Kies minstens \xE9\xE9n afvalsensor waarvan de toestand een datum is."}}watched(){return this.config.sensors.map(e=>e.entity)}read_(){let e=new Date;return this.config.sensors.map(t=>{let a=p(this.hass,t.entity);if(!a)return null;let i=H(a.state)??H(a.attributes.date)??H(a.attributes.next_date);if(!i)return null;let r=t.label??Ce(y(this.hass,t.entity,t.name)),s=et(t.label??t.entity+r),c=this.config.tones?.[t.entity];return{label:r,date:i,days:U(e,i),tone:k(c??t.tone??s.tone),icon:t.icon??s.icon}}).filter(t=>t&&t.days>=0).sort((t,a)=>t.date-a.date)}template(){let e=this.config;return e.bare&&this.setAttribute("bare",""),`
      <div class="card surface">
      ...
      </div>`}paint(){let e=this.read_(),t=this.$(".hero"),a=this.$(".list"),i=this.$(".empty");if(i.hidden=e.length>0,t&&(t.hidden=e.length===0,e.length)){let r=e[0];t.style.setProperty("--tone",r.tone),this.setAttribute("urgency",r.days===0?"today":r.days===1?"tomorrow":"later");let s=t.querySelector(".bin");s.dataset.icon!==r.icon&&(s.dataset.icon=r.icon,s.innerHTML=b(r.icon,"bin")),this.text(t.querySelector(".eyebrow"),Z(r.date)),this.text(t.querySelector(".big"),r.label),this.text(t.querySelector(".n"),r.days===0?"nu":String(r.days)),this.text(t.querySelector(".u"),r.days===0?"aan de weg":r.days===1?"dag":"dagen")}if(a){let r=this.config.show_hero===!1?e:e.slice(1),s=r.map(c=>`${c.label}${+c.date}`).join("|");if(a.dataset.sig===s)return;a.dataset.sig=s,a.innerHTML=r.map(c=>{let l=Z(c.date),h=c.days<=6?`<small>${Se(c.date)}</small>`:"";return`
        <div class="r" style="--tone:${c.tone}">
          <i></i><span>${c.label}</span>
          <span class="d">${l}${h}</span>
        </div>`}).join("")}}rows_(){let e=this.config?.sensors?.length??1;return this.config?.show_list===!1?1:this.config?.show_hero===!1?Math.max(1,q(20+e*33)):Math.max(2,e)}getCardSize(){return this.rows_()}getGridOptions(){let e=this.rows_();return{columns:12,rows:e,min_columns:6,min_rows:e,max_rows:e}}static getConfigElement(){return document.createElement("domotiapp-waste-card-editor")}static getStubConfig(e){return{sensors:Object.keys(e?.states??{}).filter(a=>/afval|waste|trash|garbage|ophaal/i.test(a)&&a.startsWith("sensor.")).filter(a=>H(e.states[a]?.state)).slice(0,6),title:"Afvalkalender"}}},fe=class extends f{defaults(){return{show_hero:!0,show_list:!0}}setConfig(e){let t={...e};for(let[a,i]of Object.entries(e.tones??{}))t[`kleur:${a}`]=i;delete t.tones,super.setConfig(t)}serialize(e){let t={...e},a={};for(let i of Object.keys(t))i.startsWith("kleur:")&&(t[i]&&(a[i.slice(6)]=t[i]),delete t[i]);return Object.keys(a).length?t.tones=a:delete t.tones,t}schema(){let e=this.config_?.sensors??[],t=Object.keys(M).map(a=>({value:a,label:N[a]??a}));return[{name:"sensors",selector:{entity:{domain:"sensor",multiple:!0}}},...e.map(a=>typeof a=="string"?a:a.entity).filter(Boolean).map(a=>({name:`kleur:${a}`,selector:d.select(t)}))]}label(e){if(e.name.startsWith("kleur:")){let t=e.name.slice(6),a=this.hass?.states?.[t]?.attributes?.friendly_name??t;return`Kleur voor ${Ce(a)||a}`}return{sensors:"Afvalsensoren",show_hero:"Eerstvolgende uitlichten",show_list:"Overige data tonen",bare:"Zonder kaartrand"}[e.name]??super.label(e)}helper(e){if(e.name==="sensors")return"Sensoren waarvan de toestand een datum is, bijvoorbeeld 18-08-2026. De kaart sorteert zelf; laat een kleur leeg om de bakkleur op de naam te laten kiezen."}};x("domotiapp-waste-card-editor",fe);w("domotiapp-waste-card",ge,{name:"DomotiApp Afvalkalender",description:"Eerstvolgende ophaling als hero, de rest eronder. Kleur per fractie."});var tt="0.1.4";console.info(`%c DOMOTIAPP-CARDS %c ${tt} `,"background:#026fa1;color:#e8e4de;font-weight:600;border-radius:3px 0 0 3px;padding:2px 6px","background:#12120f;color:#e8e4de;border-radius:0 3px 3px 0;padding:2px 6px");export{tt as VERSION};
